...
{
    Set_Value("AddYearPrefix", AddYearPrefix);
}
/** Get Append Year as Prefix.
@return Append the Year to the Prefix in document number */
public Boolean IsAddYearPrefix()
{
    Object oo = Get_Value("AddYearPrefix");
    if (oo != null)
    {
        if (oo.GetType() == typeof(bool))
            return Convert.ToBoolean(oo);
