...
namespace MarketSvc.Classes
{
    ...
    public class DatabaseMetaData : IDisposable
    {
        ...
        internal string GetForeignColumnName(DataRow dr)
        {
            if (DatabaseType.IsOracle)
            {
               return  DB.ExecuteScalar("SELECT column_name FROM user_cons_columns WHERE constraint_name='" + dr["FOREIGN_KEY_CONSTRAINT_NAME"].ToString() + "'").ToString();
            }
            else if (DatabaseType.IsPostgre)
            {
                return DB.ExecuteScalar("SELECT column_name FROM information_schema.key_column_usage WHERE constraint_name='" + dr["CONSTRAINT_NAME"].ToString() + "'").ToString();
            }
            return "";
        }
