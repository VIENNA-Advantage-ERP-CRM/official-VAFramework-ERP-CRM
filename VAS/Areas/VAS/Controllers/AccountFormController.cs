...
namespace VIS.Controllers
{
    public class AccountFormController : Controller
    {
        ...
        /// <returns></returns>
        public JsonResult Save(string VAF_Client_ID, string VAF_Org_ID, string C_AcctSchema_ID, string AD_Account_ID, string C_SubAcct_ID, string M_Product_ID,
            string C_BPartner_ID, string VAF_OrgTrx_ID, string C_LocFrom_ID, string C_LocTo_ID, string C_SRegion_ID, string C_Project_ID, string C_Campaign_ID,
            ...
            string UserElement5_ID, string UserElement6_ID, string UserElement7_ID, string UserElement8_ID, string UserElement9_ID, string Alias)
        {
            if (Session["Ctx"] != null)
            {
                var ctx = Session["ctx"] as Ctx;               
                AccountFormModel obj = new AccountFormModel();                
                var value = obj.SaveAccount(ctx, Convert.ToInt32(VAF_Client_ID), Convert.ToInt32(VAF_Org_ID), Convert.ToInt32(C_AcctSchema_ID),
                    Convert.ToInt32(AD_Account_ID), Convert.ToInt32(C_SubAcct_ID), Convert.ToInt32(M_Product_ID), Convert.ToInt32(C_BPartner_ID), Convert.ToInt32(VAF_OrgTrx_ID), 
