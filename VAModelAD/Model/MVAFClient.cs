...
{
    public class MVAFClient: VAdvantage.Model.X_VAF_Client
    {

        private static CCache<int, MVAFClient> s_cache = new CCache<int, MVAFClient>("VAF_Client_AD", 3);
        ...
        public MVAFClient(Ctx ctx, DataRow dr, Trx trx):base(ctx,dr,trx)
        {
        }

        internal static MVAFClient Get(Ctx ctx)
        {
            return Get(ctx, ctx.GetVAF_Client_ID());
        }

        internal static MVAFClient Get(Ctx ctx, int VAF_Client_ID)
        {
	 ...
	 */
        ...
        internal static MVAFClient[] GetAll(Ctx ctx)
        {
        ...
        {
            MVAFAttachment mAttach = MVAFAttachment.Get(p_ctx, VAF_TableView_ID, Record_ID);
