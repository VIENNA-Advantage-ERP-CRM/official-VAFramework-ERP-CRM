...
{
    public class MVAFPrintRptLayout : X_VAF_Print_Rpt_Layout
    {
        /** Cached Formats						*/
        static private CCache<int, MVAFPrintRptLayout> s_formats = new CCache<int, MVAFPrintRptLayout>("VAF_Print_Rpt_Layout", 30);
        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFPrintRptLayout).FullName);
        ...
        public MVAFPrintRptLayout(Ctx ctx, int VAF_Print_Rpt_Layout_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVAFPrintRptLayout(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
            _ctx = ctx;
            _language = Env.GetLanguage(ctx);
        ...
        public static MVAFPrintRptLayout Copy(Ctx ctx, int from_VAF_Print_Rpt_Layout_ID,
            int to_VAF_Print_Rpt_Layout_ID, int to_Client_ID)
        {
            ...
            //
            MVAFPrintRptLayout from = new MVAFPrintRptLayout(ctx, from_VAF_Print_Rpt_Layout_ID, null);
            MVAFPrintRptLayout to = new MVAFPrintRptLayout(ctx, to_VAF_Print_Rpt_Layout_ID, null);		//	could be 0
            MVAFPrintRptLayout.CopyValues(from, to);
            //	New
        ...
        public static MVAFPrintRptLayout Copy(Ctx ctx, int from_VAF_Print_Rpt_Layout_ID, int to_VAF_Print_Rpt_Layout_ID)
        {
        ...
        static public MVAFPrintRptLayout Get(Ctx ctx, int VAF_Print_Rpt_Layout_ID, bool readFromDisk)
        {
            int key = VAF_Print_Rpt_Layout_ID;
            MVAFPrintRptLayout pf = null;
            if (!readFromDisk)
                pf = (MVAFPrintRptLayout)s_formats[key];
            ...
            if (pf == null)
            {
                pf = new MVAFPrintRptLayout(ctx, VAF_Print_Rpt_Layout_ID, null);
                pf.GetCtx().SetContext("#TimezoneOffset", ctx.GetContext("#TimezoneOffset"));
        ...
        static public MVAFPrintRptLayout Get(Ctx ctx, int VAF_ReportView_ID, int VAF_TableView_ID)
        {
            MVAFPrintRptLayout retValue = null;
            ...
            try
            {
                ...
                {
                    retValue = new MVAFPrintRptLayout(ctx, dr, null);
                }
            }
            catch (Exception e)
            {
        ...
        /** Items							*/
        private MVAFPrintRptLItem[] _items = null;

        /** Translation View Language		*/
        private String _translationViewLanguage = null;


        private MVAFPrintRptTblLayout _tFormat;
        ...
        private void SetItems(MVAFPrintRptLItem[] items)
        {
            if (items != null)
                _items = items;
        }	//	setItems
        ...
        static private MVAFPrintRptLItem[] CopyItems(MVAFPrintRptLayout fromFormat, MVAFPrintRptLayout toFormat)
        {
            List<MVAFPrintRptLItem> list = new List<MVAFPrintRptLItem>();

            MVAFPrintRptLItem[] items = fromFormat.GetItems();
            for (int i = 0; i < items.Length; i++)
            {
                MVAFPrintRptLItem pfi = items[i].CopyToClient(toFormat.GetVAF_Client_ID(), toFormat.Get_ID());
                if (pfi != null)
                    list.Add(pfi);
            }
            //
            MVAFPrintRptLItem[] retValue = new MVAFPrintRptLItem[list.Count];
        ...
        public MVAFPrintRptLItem GetItem(int index)
        {
        ...
        /// </summary>
        /// <returns>items</returns>
        private MVAFPrintRptLItem[] GetItems()
        {
            List<MVAFPrintRptLItem> list = new List<MVAFPrintRptLItem>();
            ...
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    MVAFPrintRptLItem pfi = new MVAFPrintRptLItem(Env.GetContext(), dr, Get_TrxName());
        ...
        static private void CopyTranslationItems(MVAFPrintRptLItem[] fromItems, MVAFPrintRptLItem[] toItems)
        {
...
.Append(") WHERE VAF_Print_Rpt_LItem_ID=").Append(toID).Append(" AND EXISTS").Append("(")
        ...
        /// <returns>format</returns>
        public static MVAFPrintRptLayout CopyToClient(Ctx ctx, int VAF_Print_Rpt_Layout_ID, int to_Client_ID)
        {
        ...
        /// <param name="format">format</param>
        /// <returns>items</returns>
        static private MVAFPrintRptLItem[] CreateItems(Ctx ctx, MVAFPrintRptLayout format, int vaf_tab_ID,bool isMRSeq=false)
        {
            List<MVAFPrintRptLItem> list = new List<MVAFPrintRptLItem>();
            ...
            if (isMRSeq)
            {
                try
                {
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            MVAFPrintRptLItem pfi = MVAFPrintRptLItem.CreateFromColumn(format, Util.GetValueOfInt(ds.Tables[0].Rows[i]["VAF_Column_ID"]), Util.GetValueOfInt(ds.Tables[0].Rows[i]["VAF_Field_ID"]), i + 1, isMESeqDefined);
                            if (pfi != null)
                            {
                                list.Add(pfi);
                            }
            ...
            if (runOldCode)
            {
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        MVAFPrintRptLItem pfi = MVAFPrintRptLItem.CreateFromColumn(format, Utility.Util.GetValueOfInt(dr[0].ToString()), seqNo++);
                        if (pfi != null)
                        {
                            list.Add(pfi);
                        }
            ...
            if (list.Count == 0)
            {
                ...
                try
                {
                    ...
                    while (idr.Read())
                    {
                        MVAFPrintRptLItem pfi = MVAFPrintRptLItem.CreateFromColumn(format, Utility.Util.GetValueOfInt(idr[0].ToString()), seqNo++);
                        if (pfi != null)
                        {
                            list.Add(pfi);
                        }
        ...
        static public MVAFPrintRptLayout CreateFromTable(Ctx ctx, int VAF_TableView_ID, object VAF_Tab_ID,bool isMRSeq=false)
        {
        ...
        /// <param name="VAF_TableView_ID">VAF_TableView_ID</param>
        /// <returns>print format</returns>
        static public MVAFPrintRptLayout CreateFromTable(Ctx ctx, int VAF_TableView_ID,bool isMRSeq=false)
        {
        ...
        public MVAFPrintRptTblLayout GetTableFormat()
        {
            if (_tFormat == null)
                _tFormat = MVAFPrintRptTblLayout.Get(GetCtx(), GetVAF_Print_Rpt_TblLayout_ID(), GetVAF_Print_Rpt_Font_ID());
            return _tFormat;
        }	//	getTableFormat
        ...
        /// <param name="VAF_Print_Rpt_Layout_ID">VAF_Print_Rpt_Layout_ID</param>
        /// <returns>print format</returns>
        static public MVAFPrintRptLayout CreateFromTable(Ctx ctx, int VAF_TableView_ID, int VAF_Print_Rpt_Layout_ID)
        {
            int VAF_Client_ID = ctx.GetVAF_Client_ID();

            MVAFPrintRptLayout pf = new MVAFPrintRptLayout(ctx, VAF_Print_Rpt_Layout_ID, null);
        ...
        /// <param name="VAF_Print_Rpt_Layout_ID">VAF_Print_Rpt_Layout_ID</param>
        /// <returns>print format</returns>
        static public MVAFPrintRptLayout CreateFromTable(Ctx ctx, int VAF_TableView_ID, int VAF_Print_Rpt_Layout_ID, int VAF_Tab_ID,bool IsMRSeq=false)
        {
            int VAF_Client_ID = ctx.GetVAF_Client_ID();
        ...
        /// <returns>print format</returns>
        static public MVAFPrintRptLayout CreateFromReportView(Ctx ctx, int VAF_ReportView_ID, String ReportName)
        {
            int VAF_Client_ID = ctx.GetVAF_Client_ID();

            MVAFPrintRptLayout pf = new MVAFPrintRptLayout(ctx, 0, null);
        ...
        new public void SetVAF_Print_Rpt_TblLayout_ID(int VAF_Print_Rpt_TblLayout_ID)
        {
            ...
            _tFormat = MVAFPrintRptTblLayout.Get(GetCtx(), VAF_Print_Rpt_TblLayout_ID, GetVAF_Print_Rpt_Font_ID());
