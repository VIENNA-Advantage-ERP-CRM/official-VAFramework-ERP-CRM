...
namespace VAdvantage.Process
{
    public class NativeSequence : SvrProcess
    {
        ...
        protected override String DoIt()
        {
            ...
            String sql = "SELECT Value FROM VAF_SysConfig"
            ...
            try
            {
                CreateSequence("VAF_Record_Seq", null);
        ...
        private void SetSystemNativeSequence(bool value)
        {
            if (value)
            {
                DB.ExecuteQuery("UPDATE VAF_SysConfig SET Value='Y' WHERE Name='SYSTEM_NATIVE_SEQUENCE'");
            }
            else
            {
                DB.ExecuteQuery("UPDATE VAF_SysConfig SET Value='N' WHERE Name='SYSTEM_NATIVE_SEQUENCE'");
            }
        ...
        //            tblresult = false;
        //            totalTables = 0;
        //            string sql = @"select t1.VAF_Record_Seq_id, upper(t1.name) as NAME, t1.currentnext, t1.currentnextsys, t1.incrementno
        ...
        //from VAF_Record_Seq t1 inner join vaf_tableview t2 on t1.name = t2.name 
        ...
        //                        sqlDB.Append("execute immediate ('update VAF_Record_Seq set isnativesequence = ''Y'' where upper(name)=upper(''");
        //                        sqlDB.Append(Convert.ToString(Name).Trim());
        //                        sqlDB.Append("'')');");
