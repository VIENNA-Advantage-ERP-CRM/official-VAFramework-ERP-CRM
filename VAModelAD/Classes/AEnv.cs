...
namespace VAdvantage.Classes
{
    ...
    public class AEnv
    {
       ...
       // private static bool? _workflow = null;
        ...
        public static GridWindowVO GetMWindowVO(Ctx ctx, int windowNo, int AD_Window_ID, int VAF_MenuConfig_ID)
        {
            log.Config("Window=" + windowNo + ", AD_Window_ID=" + AD_Window_ID);
            GridWindowVO mWindowVO = null;
            string format = string.Format("{0}{1}{2}{3}", AD_Window_ID, ctx.GetVAF_Client_ID(), ctx.GetVAF_Role_ID(), ctx.GetVAF_Language());          //JID_1238: Addedd language to reset cache
            ...
            if (mWindowVO == null)
            {
                ...
                mWindowVO = GridWindowVO.Create(ctx, windowNo, AD_Window_ID, VAF_MenuConfig_ID);
                if (mWindowVO != null)
