...
{
    public class MVAFJInstance : X_VAF_JInstance
    {
        ...
        MVAFJInstancePara[] _parameter = null;
        //	Log Entries				
        private List<MVAFJInstanceLog> _log = new List<MVAFJInstanceLog>();
        ...
        public MVAFJInstance(Ctx ctx, int VAF_JInstance_ID, string ignored)
        ...
        /// <param name="ignored">no transaction support</param>
        public MVAFJInstance(Ctx ctx, DataRow dr, String ignored)
            : base(ctx, dr, null)
        {
        }
        ...
        public MVAFJInstance(MVAFJob process, int Record_ID)
            : this(process.GetCtx(), 0, null)
        {
            ...
            MVAFJobPara[] para = process.GetParameters();
            for (int i = 0; i < para.Length; i++)
            {
                MVAFJInstancePara pip = new MVAFJInstancePara(this, para[i].GetSeqNo());
        ...
        /// <param name="Record_ID">record</param>
        public MVAFJInstance(Ctx ctx, int VAF_Job_ID, int Record_ID)
            : this(ctx, 0, null)
        {
        ...
        public MVAFJInstancePara[] GetParameters()
        {
            if (_parameter != null)
                return _parameter;

            List<MVAFJInstancePara> list = new List<MVAFJInstancePara>();
            ...
            {
                list.Add(new MVAFJInstancePara(GetCtx(), dr, null));
            }

            _parameter = new MVAFJInstancePara[list.Count];
        ...
        public MVAFJInstanceLog[] GetLog()
        {
            //	load it from DB
            _log.Clear();
            ...
            try
            {
                ...
                {
                    _log.Add(new MVAFJInstanceLog(dr));
                }
            }
            catch (Exception e)
            {
            ...
            MVAFJInstanceLog[] retValue = new MVAFJInstanceLog[_log.Count];
        ...
        {
            MVAFJInstanceLog logEntry = new MVAFJInstanceLog(GetVAF_JInstance_ID(), _log.Count + 1,
        ...
        protected override bool BeforeSave(bool newRecord)
        {
            ...
            {
                MVAFSession session = MVAFSession.Get(GetCtx(), true);
        ...
        protected override bool AfterSave(bool newRecord, bool success)
        {
                ...
                && Is_ValueChanged("IsProcessing"))
            {
                ...
                MVAFJob prc = MVAFJob.Get(GetCtx(), GetVAF_Job_ID());
