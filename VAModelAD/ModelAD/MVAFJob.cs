...
{
    public class MVAFJob : X_VAF_Job
    {
        private static CCache<int, MVAFJob> _cache = new CCache<int, MVAFJob>("VAF_Job", 20);

        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFJob).FullName);
        ...
        public static MVAFJob Get(Ctx ctx, int VAF_Job_ID)
        {
            int key = VAF_Job_ID;
            MVAFJob retValue = null;
            if(_cache.ContainsKey(key))
                retValue = (MVAFJob)_cache[key];
            if (retValue != null)
            {
                retValue.p_ctx = ctx;
                return retValue;
            }
            retValue = new MVAFJob(ctx, VAF_Job_ID, null);
        ...
        public static MVAFJob GetByValue(Ctx ctx, string value)
        {
            MVAFJob retValue = null;
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        retValue = new MVAFJob(ctx, dr, null);
                        //	Save in cache
        ...
        /// <returns>MProcess</returns>
        public static MVAFJob GetFromMenu(Ctx ctx, int VAF_MenuConfig_ID)
        {
            MVAFJob retValue = null;
            String sql = "SELECT * FROM VAF_Job p "
        ...
        public MVAFJob(Ctx ctx, DataRow rs, Trx ignored)
            : base(ctx, rs, null)
        {
        ...
        /// <param name="VAF_Job_ID">ad process id</param>
        /// <param name="ignored">no transaction</param>
        public MVAFJob(Ctx ctx, int VAF_Job_ID, Trx ignored)
            : base(ctx, VAF_Job_ID, null)
        {
            if (VAF_Job_ID == 0)
            {
        ...
        private MVAFJobPara[] _parameters = null;

        /// <summary>
        /// Get Parameters
        /// </summary>
        /// <returns>Parameters</returns>
        public MVAFJobPara[] GetParameters()
        {
            if (_parameters != null)
                return _parameters;
            List<MVAFJobPara> list = new List<MVAFJobPara>();
            //
            ...
            try
            {
                ...
                {
                    DataRow dr = dt.Rows[i];
                    list.Add(new MVAFJobPara(GetCtx(), dr, null));
                }
            }
            catch (Exception e)
            {
            ...
            //
            _parameters = new MVAFJobPara[list.Count];
        ...
        public MVAFJobPara GetParameter(String name)
        {
            GetParameters();
         ...
         */
        public MVAFJInstance ProcessIt(int Record_ID)
        {
            MVAFJInstance pInstance = new MVAFJInstance(this, Record_ID);
            //	Lock
            ...
            //	Unlock
            pInstance.SetResult(ok ? MVAFJInstance.RESULT_OK : MVAFJInstance.RESULT_ERROR);
        ...
        public bool ProcessIt(ProcessInfo pi, Trx trx)
        {
            ...
            {
                MVAFJInstance pInstance = new MVAFJInstance(this, pi.GetRecord_ID());
                //	Lock
                pInstance.SetIsProcessing(true);
                pInstance.Save();
            }
        ...
        private bool StartProcess(String ProcedureName, MVAFJInstance pInstance)
        {
            ...
            catch (Exception e)
            {
                ...
                pInstance.SetResult(MVAFJInstance.RESULT_ERROR);
                pInstance.SetErrorMsg(e.Message);
                return false;
            }
            pInstance.SetResult(MVAFJInstance.RESULT_OK);
            return true;
        }
	 ...
	 */
        ...
        protected override bool AfterSave(bool newRecord, bool success)
        {
            if (newRecord)	//	Add to all automatic roles
            {
                ...
                {
                    pa = new MVAFJobRights(this, roles[i].GetVAF_Role_ID());
                    pa.Save();
                    pa = null;
                }
            }
                ...
                || Is_ValueChanged("Description") || Is_ValueChanged("Help"))
            {
                ...
                X_VAF_WFlow_Node[] nodes = MVAFScreen.GetWFNodes(GetCtx(), "VAF_Job_ID=" + GetVAF_Job_ID());
