...
{
    public class MVAFRecordSeq : X_VAF_Record_Seq
    {
        ...
        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFRecordSeq).FullName);
        ...
        public MVAFRecordSeq(Ctx ctx, int VAF_Record_Seq_ID, Trx trxName)
         ...
         */
        public MVAFRecordSeq(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {

        }
        public MVAFRecordSeq(Ctx ctx, IDataReader idr, Trx trxName)
         ...
         *	@param trxName transaction
         */
        public MVAFRecordSeq(Ctx ctx, int VAF_Client_ID, String tableName, Trx trxName)
            : this(ctx, 0, trxName)
        {
         ...
         *	@param trxName trx
         */
        public MVAFRecordSeq(Ctx ctx, int VAF_Client_ID, String sequenceName, int StartNo, Trx trxName)
            : this(ctx, 0, trxName)
        {
...
#pragma warning disable 612, 618
        ...
        public static String GetDocumentNo(int VAB_DocTypes_ID, Trx trxName, Ctx ctx, Boolean definite, PO po)
        {
            ...
            lock (GetLock(dt.GetDocNoSequence_ID().ToString()))
            {
                ...
                MVAFRecordSeq seq = new MVAFRecordSeq(ctx, seqID, trxName);
        ...
        public static String GetDocumentNoFromSeq(MVAFRecordSeq seq, Trx trxName, PO po)
        {
            //	Check viennaSys
        ...
        public static String GetDocumentNo(int VAF_Client_ID, String TableName, Trx trxIn, Ctx ctx)
        {
            ...
            lock (GetLock(TableName))
            {
                ...
                try
                {
                    ...
                    {
                        MVAFRecordSeq seq = new MVAFRecordSeq(ctx, VAF_Client_ID, TableName, trx);
                        next = seq.GetNextID();
        ...
        //        //DataSet ds1 = BaseLibrary.DataBase.DB.ExecuteDataset(selectSQL,null,null);
        //        //dap.Fill(ds);
        ...
        public static String GetDocumentNo(String TableName, Trx trxName, Ctx ctx, PO po)
        {
            ...
            MVAFRecordSeq seq = MVAFRecordSeq.Get(ctx, TableName, trxName, false);
            if (seq == null || seq.Get_ID() == 0)
            {
                if (!MVAFRecordSeq.CreateTableSequence(ctx, TableName, trxName, false))
                {
                    s_log.Severe("Could not create table sequence");
                    return null;
                }

                seq = MVAFRecordSeq.Get(ctx, TableName, trxName, false);
                if (seq == null || seq.Get_ID() == 0)
                {
        ...
        {
            MVAFRecordSeq seq = new MVAFRecordSeq(ctx, 0, trxName);
        ...
        public static Boolean CreateTableSequence(Ctx ctx, String TableName, Trx trxName, Boolean tableID)
        {
            if (tableID)
            {
                return MVAFRecordSeq.CreateTableSequence(ctx, TableName, trxName);
            }
            else
            {
                MVAFRecordSeq seq = new MVAFRecordSeq(ctx, 0, trxName);
        ...
        {
            MVAFRecordSeq seq = Get(ctx, TableName, trxName);
        ...
        public static MVAFRecordSeq Get(Ctx ctx, String tableName, Trx trxName)
        {
            String sql = "SELECT * FROM VAF_Record_Seq "
                + "WHERE UPPER(Name)='" + tableName.ToUpper() + "'"
                + " AND IsTableID='Y'";
            MVAFRecordSeq retValue = null;
            DataTable dt = null;
            ...
            try
            {
                ...
                {
                    dr = dt.Rows[i];
                    retValue = new MVAFRecordSeq(ctx, dr, null);
                    if (i > 0)
                    {
        ...
        /// <param name="tableID">Used for Record ID</param>
        /// <returns></returns>
        public static MVAFRecordSeq Get(Ctx ctx, String tableName, Trx trxName, Boolean tableID)
        {
            if (!tableID)
            {
            ...
            try
            {
                ...
                {
                    dr = dt.Rows[i];
                    retValue = new MVAFRecordSeq(ctx, dr, trxName);
                    if (i > 0)
                    {
                        s_log.Log(Level.SEVERE, "More then one sequence for " + tableName);
                    }
        ...
        public static Boolean CheckClientSequences(Ctx ctx, int VAF_Client_ID, Trx trxName)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < totCount; i++)
                {
                    ...
                    MVAFRecordSeq seq = new MVAFRecordSeq(ctx, VAF_Client_ID, tableName, trxName);
                    if (seq.Save())
                    {
                        counter++;
                    }
        ...
        public bool ValidateTableIDValue()
        {
            ...
            //
            MVAFSystem system = MVAFSystem.Get(GetCtx());
            int IDRangeEnd = 0;
