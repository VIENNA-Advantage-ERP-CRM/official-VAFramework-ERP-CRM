...
{
    public class MVAFImage : X_VAF_Image
    {
        /// <summary>
        ...
        public static MVAFImage Get(Ctx ctx, int VAF_Image_ID)
        {
            if (VAF_Image_ID == 0)
                return new MVAFImage(ctx, VAF_Image_ID, null);
            //
            int key = VAF_Image_ID;
            MVAFImage retValue = s_cache[key];
            if (retValue != null)
                return retValue;
            retValue = new MVAFImage(ctx, VAF_Image_ID, null);
        ...
        /**	Cache						*/
        private static CCache<int, MVAFImage> s_cache = new CCache<int, MVAFImage>("VAF_Image", 20);
        ...
        public MVAFImage(Ctx ctx, int VAF_Image_ID, Trx trxName)
        ...
        /// <param name="rs"></param>
        /// <param name="trxName"></param>
        public MVAFImage(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        }	//	MImage
	 ...
	 */
        ...
        private void DeleteThumbnail(string x, string y, string imageName)
        {
            try
            {
                string filepath = "";
                if (x == "0")
                    filepath = Path.Combine(HostingEnvironment.MapPath(@"~/Images"), imageName);
                else
                    filepath = Path.Combine(HostingEnvironment.MapPath(@"~/Images"), "Thumb" + x + "x" + y + "/" + imageName);
