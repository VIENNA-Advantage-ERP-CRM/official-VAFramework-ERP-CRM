...
{
    public class MVAFSystem : X_VAF_System
    {
        ////System - cached					
        private static MVAFSystem _system = null;
        /**	Cache					*/
        private static CCache<int, MVAFSystem> cache = new CCache<int, MVAFSystem>("VAF_System", 30, 60);
        ...
        public static MVAFSystem Get(Ctx ctx)
        {
            _system = (MVAFSystem)cache[101];
            ...
            if (_system == null)
            {
                ...
                try
                {
                    ...
                    if (idr.Read())
                    {
                        _system = new MVAFSystem(ctx, idr, null);
                    }
                    idr.Close();
                }
                catch (Exception ex)
        ...
        public MVAFSystem(Ctx ctx, int ignored, Trx mtrxName)
            : base(ctx, 0, mtrxName)
        {
            Trx trxName = null;
            if (ignored >= 0)
        ...
        public MVAFSystem(Ctx ctx, IDataReader idr, Trx trxName)
        ...
        public String GetStatisticsInfo(bool recalc)
        {
            ...
            if (s == null || recalc)
            {
                ...
                String sql = "SELECT 'C'||(SELECT " + count + " FROM VAF_Client)"
                    ...
                    + " ||'P'||(SELECT " + count + " FROM VAM_Product)"
