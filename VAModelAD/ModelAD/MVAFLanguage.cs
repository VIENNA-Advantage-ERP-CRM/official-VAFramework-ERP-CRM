...
{
    public class MVAFLanguage : X_VAF_Language
    {
        #region Private Variable
        //Logger
        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFLanguage).FullName);
        //Locale
        ...
        public static MVAFLanguage Get(Ctx ctx, String VAF_Language, Trx trxName)
        {
            MVAFLanguage lang = null;
            ...
            try
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    lang = new MVAFLanguage(ctx, rs, trxName);
                }
                ds = null;
            }
            catch (Exception ex)
        ...
        /// <param name="trxName">trx</param>
        /// <returns>language or null</returns>
        public static MVAFLanguage Get(Ctx ctx, int VAF_Language_ID, Trx trxName)
        {
            MVAFLanguage lang = null;
        ...
        public static MVAFLanguage Get(Ctx ctx, VAdvantage.Login.Language lang)
        {
        ...
        /// <returns>language or null</returns>
        public static MVAFLanguage Get(Ctx ctx, string VAF_Language)
        {
            MVAFLanguage lang = null;
            String sql = "SELECT * FROM VAF_Language WHERE VAF_Language=" + VAF_Language;
        ...
        public MVAFLanguage(Ctx ctx, int VAF_Language_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVAFLanguage(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
            //super(ctx, rs, trxName);
        }
        public MVAFLanguage(Ctx ctx, IDataReader rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
            //super(ctx, rs, trxName);
        }
