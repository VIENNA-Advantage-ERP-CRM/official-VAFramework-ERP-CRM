...
namespace VAdvantage.Model
{
    ...
    /// of a record from "Attachment" window
    /// </summary>
    public class MVAFAttachment : X_VAF_Attachment
    {

        /**	Static Logger	*/
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAFAttachment).FullName);
        ...
        public MVAFAttachment(Ctx ctx, int VAF_TableView_ID, int Record_ID, Trx trxName)
            : base(ctx, 0, trxName)
        {
            //this(ctx, 0, trxName);
            // set table id
        ...
        /// <param name="trxName">transaction</param>
        public MVAFAttachment(Ctx ctx, int VAF_Attachment_ID, Trx trxName)
        ...
        /// <param name="VAF_Attachment_ID">VAF_Attachment_ID</param>
        /// <param name="trxName">transaction</param>
        public MVAFAttachment(Ctx ctx, int VAF_Attachment_ID, Trx trxName, string password, int vaf_client_ID)
            : base(ctx, VAF_Attachment_ID, trxName)
        {
            error = new StringBuilder();
            Password = password;
        ...
        /// <param name="trxName">transaction</param>
        public MVAFAttachment(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
            error = new StringBuilder();
            TryGetLines();
        ...
        public static MVAFAttachment Get(Ctx ctx, int VAF_TableView_ID, int Record_ID)
        {
            MVAFAttachment retValue = null;
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        retValue = new MVAFAttachment(ctx, dr, null);
                    }
                }
            }
            catch (Exception e)
            ...
            try
            {
                ...
                if (GetFileLocation() == FILELOCATION_WebService)
                {
                    ...
                    try
                    {
                        ...
                        MVAFAttachmentReference attRef = new MVAFAttachmentReference(GetCtx(), 0, Get_Trx());
