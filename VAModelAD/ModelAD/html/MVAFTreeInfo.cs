...
{
    public class MVAFTreeInfo : X_VAF_TreeInfo
    {
        #region "Mtree TreeNode

        /**	Cache						*/
        private static CCache<int, MVAFTreeInfo> s_cache = new CCache<int, MVAFTreeInfo>("VAF_TreeInfo", 10);
        ...
        //	Logger	
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAFTreeInfo).FullName);
        ...
        public MVAFTreeInfo(Ctx ctx, int VAF_TreeInfo_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, int VAF_TreeInfo_ID, bool editable, bool clientTree, Trx trxName, bool onDemand, int VAF_Tab_ID, int windowNO)
        ...
        /// <param name="clientTree">the tree is displayed on the java client (not on web)</param>
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, int VAF_TreeInfo_ID, bool editable, bool clientTree, Trx trxName, bool onDemand)
            : this(ctx, VAF_TreeInfo_ID, trxName)
        {
            this.VAF_Tab_ID = VAF_Tab_ID;
            this.onDemand = onDemand;
        ...
        /// <param name="clientTree">the tree is displayed on the java client (not on web)</param>
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, int VAF_TreeInfo_ID, bool editable, bool clientTree, Trx trxName, int nodeid)
            : this(ctx, VAF_TreeInfo_ID, trxName)
        {
            this.VAF_Tab_ID = VAF_Tab_ID;
            m_editable = editable;
        ...
        /// <param name="clientTree">the tree is displayed on the java client (not on web)</param>
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, int VAF_TreeInfo_ID, bool editable, bool clientTree, Trx trxName, int nodeid, int VAF_Tab_ID)
            : this(ctx, VAF_TreeInfo_ID, trxName)
        {
            this.VAF_Tab_ID = VAF_Tab_ID;
            m_editable = editable;
        ...
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        }
        ...
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, String Name, String TreeType, Trx trxName)
            : this(ctx, 0, trxName)
        {
            SetName(Name);
        ...
        /// <param name="name">name</param>
        /// <param name="treeType">tree type</param>
        public MVAFTreeInfo(X_VAF_Client client, String name, String treeType)
        ...
        /// <param name="clientTree">the tree is displayed on the java client (not on web)</param>
        /// <param name="trxName">transaction</param>
        public MVAFTreeInfo(Ctx ctx, int VAF_TreeInfo_ID, bool editable, bool clientTree, Trx trxName)
            : this(ctx, VAF_TreeInfo_ID, trxName)
        {
            m_editable = editable;
            int VAF_UserContact_ID = ctx.GetVAF_UserContact_ID();
        ...
        public static MVAFTreeInfo Get(Ctx ctx, int VAF_TreeInfo_ID, Trx trxName)
        {
            int key = VAF_TreeInfo_ID;
            MVAFTreeInfo retValue = (MVAFTreeInfo)s_cache[key];
            if (retValue != null)
                return retValue;
            retValue = new MVAFTreeInfo(ctx, VAF_TreeInfo_ID, trxName);
        ...
        static void FillUserTables(Trx trxName, Ctx ctx)
        {
            ...
            {
                MVAFTreeInfo xx = Get(ctx, 10, trxName);
                xx.UpdateTrees();
                FillUserTables(null, ctx);
            }
        }
        ...
        protected override bool AfterSave(bool newRecord, bool success)
        {
            ...
            if (newRecord)	//	Base Node
            {
                ...
                {
                    MVAFTreeInfoChildMenu ndMM = new MVAFTreeInfoChildMenu(this, 0);
                    ndMM.save();
                }
                else if (TREETYPE_Product.equals(treeType))
                {
                ...
                {
                    MVAFTreeInfoChild nd = new MVAFTreeInfoChild(this, 0);
                    nd.save();
                }
            }
            **/
        ...
        protected void GetNodeDetails()
        {
            ...
            if (!m_editable)	//	editable = menu/etc. window
                sql = MVAFRole.GetDefault(GetCtx(), false).AddAccessSQL(sql,
                    sourceTable, MVAFRole.SQL_FULLYQUALIFIED, m_editable);
            log.Fine(sql);
