...
namespace VAdvantage.Model
{
    ...
    /// </summary>
    public class MVAFRole : X_VAF_Role
    {
        ...
        //	Default Role 
        private static MVAFRole _defaultRole = null;
        ...
        // Log						
        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFRole).FullName);
        ...
        /**	Cache						*/
        private static CCache<int, MVAFRole> s_cache = new CCache<int, MVAFRole>("VAF_Role", 10);
        /**	Cache						*/
        ...
        /// </summary>
        /// <returns></returns>
        public static MVAFRole GetDefault(Ctx ctx)
        {
        ...
        public static MVAFRole[] GetOfClient(Ctx ctx)
        {
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    for (int i = 0; i < totCount; i++)
                    {
                        ...
                        list.Add(new MVAFRole(ctx, dr, null));
                    }
                }
            }
            catch (Exception e)
            ...
            MVAFRole[] retValue = new MVAFRole[list.Count];
     ...
     */
        public static MVAFRole[] GetOf(Ctx ctx, String whereClause)
        {
        ...
        public static MVAFRole GetDefault(Ctx ctx, bool reload)
        {
            ...
            MVAFRole role = (MVAFRole)s_cache.Get(VAF_Role_ID);
            lock (_lock)
            {
                if (reload || role == null)
                {
        ...
        /// <returns></returns>
        public static MVAFRole Get(Ctx ctx, int VAF_Role_ID, int VAF_UserContact_ID, bool reload)
        {
            s_log.Info("VAF_Role_ID=" + VAF_Role_ID + ", VAF_UserContact_ID=" + VAF_UserContact_ID + ", reload=" + reload);
            MVAFRole role = new MVAFRole(ctx, VAF_Role_ID, null);
            if (VAF_Role_ID == 0)
            {
                Trx trxName = null;
        ...
        /// <param name="VAF_Role_ID"></param>
        /// <returns></returns>
        public static MVAFRole Get(Ctx ctx, int VAF_Role_ID)
        {
            string key = VAF_Role_ID.ToString();
            Trx trxName = null;
            MVAFRole role = new MVAFRole(ctx, VAF_Role_ID, trxName);
        ...
        public MVAFRole(Ctx ctx, int VAF_Role_ID, Trx trxName)
        ...
        public MVAFRole(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
            //nothing
        }
        ...
        protected override bool AfterSave(bool newRecord, bool success)
        {
            if (newRecord && success)
            {
                ...
                MVAFUserContactRoles su = new MVAFUserContactRoles(GetCtx(), SUPERUSER_USER_ID, GetVAF_Role_ID(), Get_TrxName());
                su.Save();
                //Add Role to User
                if (GetCreatedBy() != SUPERUSER_USER_ID)
                {
                    MVAFUserContactRoles ur = new MVAFUserContactRoles(GetCtx(), GetCreatedBy(), GetVAF_Role_ID(), Get_TrxName());
                    ur.Save();
                }
                UpdateAccessRecords();
            }
        ...
        private MVAFTableViewRights[] _tableAccess = null;

        //List-Array OF Column Access
        private MVAFColumnRights[] _columnAccess = null;

        // List-Array of Record Access
        private MVAFRecordRights[] _recordAccess = null;
        // List-Array of Dependent Record Access
        private MVAFRecordRights[] _recordDependentAccess = null;
        ...
        private void LoadTableAccess(bool reload)
        {
            ...
            List<MVAFTableViewRights> list = new List<MVAFTableViewRights>();
            ...
            try
            {
                ...
                {
                    list.Add(new MVAFTableViewRights(GetCtx(), rs, Get_TrxName()));
                }
            }
            catch (Exception e)
            {
        ...
        public MVAFTableViewRights[] GetTableAccess()
        {
            return _tableAccess;
        }
        ...
        private void LoadRecordAccess(bool reload)
        {
            if (!(reload || _recordAccess == null || _recordDependentAccess == null))
                return;
            List<MVAFRecordRights> list = new List<MVAFRecordRights>();
            List<MVAFRecordRights> dependent = new List<MVAFRecordRights>();
            ...
            try
            {
                ...
                foreach (DataRow rs in dt.Rows)
                {
                    MVAFRecordRights ra = new MVAFRecordRights(GetCtx(), rs, Get_TrxName());
                    list.Add(ra);
        ...
        public MVAFRecordRights[] GetRecordAccess()
        {
            return _recordAccess;
        }
        public MVAFRecordRights[] GetRecordDependentAccess()
        {
        ...
        private void LoadOrgAccessUser(List<OrgAccess> list)
        {
            ...
            try
            {
                ...
                foreach (DataRow rs in dt.Rows)
                {
                    MVAFUserContactOrgRights oa = new MVAFUserContactOrgRights(GetCtx(), rs, Get_TrxName());
        ...
        private void LoadOrgAccessRole(List<OrgAccess> list)
        {
            ...
            try
            {
                ...
                foreach (DataRow rs in dt.Rows)
                {
                    MVAFRoleOrgRights oa = new MVAFRoleOrgRights(GetCtx(), rs, Get_TrxName());
                    LoadOrgAccessAdd(list, new OrgAccess(oa.GetVAF_Client_ID(), oa.GetVAF_Org_ID(), oa.IsReadOnly()));
                }
                dr.Dispose();
                dt.Dispose();
	 ...
	 */
        ...
        private string GetRecordWhere(int VAF_TableView_ID, string keyColumnName, bool rw)
        {
            ...
            if (!IsPersonalAccess())
            {
                ...
                {
                    string lockedIDs = MVAFPrivateRights.GetLockedRecordWhere(VAF_TableView_ID, _VAF_UserContact_ID);
                    if (lockedIDs != null)
                    {
                        if (sb.Length > 0)
                            sb.Append(" AND ");
        ...
        public bool IsRecordAccess(int VAF_TableView_ID, int Record_ID, bool ro)
        {
            ...
            for (int i = 0; i < _recordAccess.Length; i++)
            {
                MVAFRecordRights ra = _recordAccess[i];
	 ...
	 */
        public String GetIncludedRolesWhereClause(String roleColumnSQL, List<Object> param)
        {
            ...
            //}
            //
            foreach (MVAFRole role in GetIncludedRoles(true))
            {
                // if (param != null)
                //  {
        ...
        private List<MVAFRole> m_includedRoles = null;
        /**
	 * 
	 * @return unmodifiable list of included roles
	 * @see metas-2009_0021_AP1_G94
	 */
        public List<MVAFRole> GetIncludedRoles(Boolean recursive)
        {
            if (!recursive)
            {
                List<MVAFRole> list = this.m_includedRoles;
                if (list == null)
                    list = new List<MVAFRole>();
                return list;
            }
            else
            {
                ...
                {
                    foreach (MVAFRole role in m_includedRoles)
                    {
                        list.Add(role);
