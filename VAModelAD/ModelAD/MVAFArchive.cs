...
{
    public class MVAFArchive : X_VAF_Archive
    {
        public static MVAFArchive[] Get(Ctx ctx, String whereClause)
        {
            List<MVAFArchive> list = new List<MVAFArchive>();
            ...
            try
            {
                ...
                {
                    list.Add(new MVAFArchive(ctx, dr, null));
                }
            }
            catch (Exception e)
            {
            ...
            if (list.Count() == 0)
                ...
                s_log.Finer(sql);
            //
            MVAFArchive[] retValue = new MVAFArchive[list.Count()];
            retValue = list.ToArray();
            return retValue;
        }	//	get

        /**	Logger			*/
        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFArchive).FullName);

        private Int64 m_inflated = 0;
        private Int64 m_deflated = 0;

        public MVAFArchive(Ctx ctx, DataRow rs, Trx trx)
            : base(ctx, rs, trx)
        {

        }	//	MArchive

        public MVAFArchive(Ctx ctx, int VAF_Archive_ID, Trx trx)
            : base(ctx, VAF_Archive_ID, trx)
        {

        }	//	MArchive

        public MVAFArchive(Ctx ctx, IPrintInfo info, Trx trx)
            : this(ctx, 0, trx)
        {
