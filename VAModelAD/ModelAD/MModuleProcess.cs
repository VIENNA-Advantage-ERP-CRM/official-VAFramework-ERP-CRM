    ...
    public class MModuleProcess:X_VAF_ModuleProcess
    {
        ...
        protected override bool BeforeSave(bool newRecord)
        {
            if (newRecord)
            {
                ...
                if (Util.GetValueOfInt(DB.ExecuteScalar("SELECT Count(*) FROM VAF_ModuleProcess WHERE AD_Process_ID=" + GetAD_Process_ID() + " AND VAF_ModuleInfo_ID=" + GetVAF_ModuleInfo_ID())) > 0)
                {
