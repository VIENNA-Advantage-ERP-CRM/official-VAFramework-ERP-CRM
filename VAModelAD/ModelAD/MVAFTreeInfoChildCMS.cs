...
{
    public class MVAFTreeInfoChildCMS : X_VAF_TreeInfoChildCMS
    {
        /**	Static Logger	*/
   ...
   /// <returns>array of nodes</returns>
	public static MVAFTreeInfoChildCMS[] GetTree(Ctx ctx, int VAF_TreeInfo_ID, Trx trxName)
	{
		List<MVAFTreeInfoChildCMS> list = new List<MVAFTreeInfoChildCMS>();
		...
		try
		{
			...
			{
				list.Add (new MVAFTreeInfoChildCMS (ctx,dr, trxName));
			}
            dt=null;
		}
		catch (Exception e)
		...
		MVAFTreeInfoChildCMS[] retValue = new MVAFTreeInfoChildCMS[list.Count];
	...
	public static MVAFTreeInfoChildCMS Get(MVAFTreeInfo tree, int Node_ID)
	{
		MVAFTreeInfoChildCMS retValue = null;
		...
		try
		{
			...
			foreach(DataRow dr in dt.Rows)
            {
				retValue = new MVAFTreeInfoChildCMS (tree.GetCtx(), dr, tree.Get_TrxName());
            }
	...
	public MVAFTreeInfoChildCMS (Ctx ctx,DataRow dr, Trx trxName):base(ctx,dr, trxName)
	{
	...
	/// <param name="tree"> tree</param>
	/// <param name="Node_ID">node</param>
	public MVAFTreeInfoChildCMS (MVAFTreeInfo tree, int Node_ID):base(tree.GetCtx(), 0, tree.Get_TrxName())
	{
    ...
    /// <param name="tree"> tree</param>
    /// <param name="Node_ID">node</param>
    public MVAFTreeInfoChildCMS(MVAFTreeInfo tree, int Node_ID, int setSeqManually)
