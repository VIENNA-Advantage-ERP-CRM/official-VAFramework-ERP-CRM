...
{
    public class MVAFRoleOrgRights : X_VAF_Role_OrgRights
    {
        private String _clientName;
        private String _orgName;
        //Static Logger	
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAFRoleOrgRights).FullName);
        ...
        public static MVAFRoleOrgRights[] GetOfRole(Ctx ctx, int VAF_Role_ID)
        {
        ...
        /// <returns>array of Role Org Access</returns>
        public static MVAFRoleOrgRights[] GetOfClient(Ctx ctx, int VAF_Client_ID)
        {
        ...
        /// <returns>array of Role Org Access</returns>
        public static MVAFRoleOrgRights[] GetOfOrg(Ctx ctx, int VAF_Org_ID)
        {
        ...
        /// <returns>array of Role Org Access</returns>
        private static MVAFRoleOrgRights[] Get(Ctx ctx, String sql, int id)
        {
            List<MVAFRoleOrgRights> list = new List<MVAFRoleOrgRights>();
            DataTable dt = null;
            ...
            try
            {
                ...
                {
                    list.Add(new MVAFRoleOrgRights(ctx, dr, null));
                }
            }
            catch (Exception e)
            {
            ...
            MVAFRoleOrgRights[] retValue = new MVAFRoleOrgRights[list.Count];
        ...
        public MVAFRoleOrgRights(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
        }
        ...
        /// <param name="trxName">transaction</param>
        public MVAFRoleOrgRights(Ctx ctx, int ignored, Trx trxName)
            : base(ctx, 0, trxName)
        {
            if (ignored != 0)
        ...
        public MVAFRoleOrgRights(PO org, int VAF_Role_ID)
        ...
        public MVAFRoleOrgRights(MVAFRole role, int VAF_Org_ID)
        ...
        {
            int counter = 0;
            MVAFRole[] roles = MVAFRole.GetOfClient(org.GetCtx());
            for (int i = 0; i < roles.Length; i++)
            {
                if (!roles[i].IsManual())
                {
                    MVAFRoleOrgRights orgAccess = new MVAFRoleOrgRights(org, roles[i].GetVAF_Role_ID());
                    if (orgAccess.Save())
                        counter++;
                }
            }
        ...
        {
            MVAFRole role = new MVAFRole(GetCtx(), GetVAF_Role_ID(), null);
