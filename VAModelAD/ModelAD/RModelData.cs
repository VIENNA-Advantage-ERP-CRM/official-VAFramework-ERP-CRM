...
namespace VAdvantage.Model
{
    public class RModelData
    {
        ...
        public void Query(Ctx ctx, String whereClause, String orderClause)
        {
            ...
            try
            {
                ...
                idr = CoreLibrary.DataBase.DB.ExecuteReader(finalSQL, null, null);
                DataSet ds = CoreLibrary.DataBase.DB.ExecuteDataset(finalSQL, null, null);
                while (idr.Read())
                {
    ...
    //            //Statement stmt = DataBase.createStatement();
    //            //ResultSet rs = stmt.executeQuery(finalSQL);
    //            idr = BaseLibrary.DataBase.DB.ExecuteReader(finalSQL, null, null);
    //            //DataSet ds = BaseLibrary.DataBase.DB.ExecuteDataset(finalSQL, null, null);
    //            while (idr.Read())
    //            {
    //                List<Object> row = new List<Object>(size);
    //                index = 1;
