...
namespace VAdvantage.Model
{
    ...
    /// 
    /// </summary>
    public class MVAFPrintRptColour : X_VAF_Print_Rpt_Colour
    {
        //	Static Logger	
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAFPrintRptColour).FullName);
        /// <summary>
        /// Constructor
        /// </summary>
        ...
        public MVAFPrintRptColour(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        }	//	MPrintColor

        static MVAFPrintRptColour Create(Color color, String name)
        {
            MVAFPrintRptColour pc = new MVAFPrintRptColour(Env.GetContext(), 0, null);
            pc.SetName(name);
            pc.SetColor(color);
            pc.Save();
            return pc;
        ...
        static private CCache<int, MVAFPrintRptColour> _colors = new CCache<int, MVAFPrintRptColour>("VAF_Print_Rpt_Colour", 20);


        static public MVAFPrintRptColour Get(Ctx ctx, int VAF_Print_Rpt_Colour_ID)
        {
            //	if (VAF_Print_Rpt_Colour_ID == 0)
            //		return new MPrintColor (ctx, 0);
            //Info
            int key = VAF_Print_Rpt_Colour_ID;
            MVAFPrintRptColour pc = (MVAFPrintRptColour)_colors[key];
            if (pc == null)
            {
                pc = new MVAFPrintRptColour(ctx, VAF_Print_Rpt_Colour_ID, null);
                _colors.Add(key, pc);
            }
            return pc;
        }	//	get

        static public MVAFPrintRptColour Get(Ctx ctx, String VAF_Print_Rpt_Colour_ID)
        {
        ...
        public MVAFPrintRptColour(Ctx ctx, int VAF_Print_Rpt_Colour_ID, Trx trxName)
