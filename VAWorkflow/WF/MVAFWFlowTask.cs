...
{
    public class MVAFWFlowTask : X_VAF_WFlow_Task
    {
        /**	State Machine				*/
        private StateEngine _state = null;
        /**	Workflow Node				*/
        private MVAFWFlowNode _node = null;
        /** Transaction					*/
        private Trx _trx = null;
        /**	Audit						*/
        private MVAFWFlowEventLog _audit = null;
        /** Persistent Object			*/
        private PO _po = null;
        /** Document Status				*/
        ...
        /**	Static Logger	*/
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAFWFlowTask).FullName);
        ...
        public MVAFWFlowTask(Ctx ctx, int VAF_WFlow_Task_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVAFWFlowTask(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
            _state = new StateEngine(GetWFState());
            _state.SetCtx(GetCtx());
            ...
            : base(process.GetCtx(), 0, process.Get_TrxName())
        {
            ...
            MVAFWFlowNode node = MVAFWFlowNode.Get(GetCtx(), VAF_WFlow_Node_ID);
        ...
        public static MVAFWFlowTask[] Get(Ctx ctx, int VAF_TableView_ID, int Record_ID, bool activeOnly)
        {
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        list.Add(new MVAFWFlowTask(ctx, dr, null));
                    }
                }
            }
            catch (Exception e)
            ...
            MVAFWFlowTask[] retValue = new MVAFWFlowTask[list.Count];
        ...
        {
            MVAFWFlowTask[] acts = Get(ctx, VAF_TableView_ID, Record_ID, true);
        ...
        private void UpdateEventAudit()
        {
            ...
            if (_state.IsClosed())
            {
                _audit.SetEventType(MVAFWFlowEventLog.EVENTTYPE_ProcessCompleted);
                long ms = CommonFunctions.CurrentTimeMillis();//-_audit.GetCreated().getTime();
                _audit.SetElapsedTimeMS(Convert.ToDecimal(ms));

            }
            else
                _audit.SetEventType(MVAFWFlowEventLog.EVENTTYPE_StateChanged);
            _audit.Save();
        }
        ...
        public MVAFWFlowEventLog GetEventAudit()
        {
            if (_audit != null)
                return _audit;
        ...
        public MVAFWFlowNode GetNode()
        {
            if (_node == null)
                _node = MVAFWFlowNode.Get(GetCtx(), GetVAF_WFlow_Node_ID());
            return _node;
        }
        ...
        {
            return MVAFCtrlRefList.GetListName(GetCtx(), WFSTATE_VAF_Control_Ref_ID, GetWFState());
        }
        ...
        private void SetResponsible(MVAFWFlowHandler process)
        {
            ...
            MVAFWFlowIncharge resp = GetResponsible();
        ...
        public MVAFWFlowIncharge GetResponsible()
        {
            ...
            int VAF_Org_ID, bool ownDocument, bool FetchLastApprover)
        {
            ...
            while (user != null)
            {
                ...
                //	Get Roles of User
                MVAFRole[] roles = user.GetRoles(VAF_Org_ID);
                for (int i = 0; i < roles.Length; i++)
                {
                    MVAFRole role = roles[i];
                ...
                {
                    user = MVAFUserContact.Get(GetCtx(), user.GetSupervisor_ID());
                ...
                else
                {
                    ...
                    MVAFOrgDetail orgInfo = org.GetInfo();
                    //	Get Org Supervisor
                    if (orgInfo.GetSupervisor_ID() != 0)
                    {
                        user = MVAFUserContact.Get(GetCtx(), orgInfo.GetSupervisor_ID());
                    ...
                    else
                    {
                        ...
                        {
                            org = MVAFOrg.Get(GetCtx(), orgInfo.GetParent_Org_ID());
                            orgInfo = org.GetInfo();
                            if (orgInfo.GetSupervisor_ID() != 0)
                            {
                                user = MVAFUserContact.Get(GetCtx(), orgInfo.GetSupervisor_ID());
        ...
        public void Run()
        {
            ...
            try
            {
                ...
                if ((MVAFWFlowNode.ACTION_EMail.Equals(_node.GetAction()) || MVAFWFlowNode.ACTION_EMailPlusFaxEMail.Equals(_node.GetAction())) && _node.IsBackground())
                {
        ...
        private bool PerformWork(Trx trx)
        {
            ...
            /******	Sleep (Start/End)			******/
            if (MVAFWFlowNode.ACTION_WaitSleep.Equals(action))
            {
            ...
            /******	Document Action				******/
            else if (MVAFWFlowNode.ACTION_DocumentAction.Equals(action))
            {
            ...
            /******	Report						******/
            else if (MVAFWFlowNode.ACTION_AppsReport.Equals(action))
            {
                log.Fine("Report:VAF_Job_ID=" + _node.GetVAF_Job_ID());
                //	Process
                MVAFJob process = MVAFJob.Get(GetCtx(), _node.GetVAF_Job_ID());
                ...
                MVAFJInstance pInstance = new MVAFJInstance(process, GetRecord_ID());
            ...
            /******	Process						******/
            else if (MVAFWFlowNode.ACTION_AppsProcess.Equals(action))
            {
            ...
            /******	TODO Start Task				******/
            else if (MVAFWFlowNode.ACTION_AppsTask.Equals(action))
            {
                log.Warning("Task:VAF_Task_ID=" + _node.GetVAF_Task_ID());
            }

            /******	EMail						******/
            else if (MVAFWFlowNode.ACTION_EMail.Equals(action))
            {
            ...
            /******	Set Variable				******/
            else if (MVAFWFlowNode.ACTION_SetVariable.Equals(action))
            {
            ...
            else if (MVAFWFlowNode.ACTION_SubWorkflow.Equals(action))
            {
                log.Warning("Workflow:VAF_Workflow_ID=" + _node.GetVAF_Workflow_ID());
            }

            /******	User Choice					******/
            else if (MVAFWFlowNode.ACTION_UserChoice.Equals(action))
            {
                ...
                else if (_node.IsMultiApproval() && _node.GetApprovalLeval() > -1) //For Leave Request Approval
                {
                    ...
                    else if (_node.GetVAF_WFlow_Incharge_ID() > 0 || _node.GetWorkflow().GetVAF_WFlow_Incharge_ID() > 0)
                    {
                        ...
                        MVAFWFlowIncharge resp = new MVAFWFlowIncharge(GetCtx(), VAF_WFlow_Incharge_ID, Get_TrxName());
            ...
            /******	User Workbench				******/
            else if (MVAFWFlowNode.ACTION_UserWorkbench.Equals(action))
            {
                log.Fine("Workbench:?");
                return false;
            }
            /******	User Form					******/
            else if (MVAFWFlowNode.ACTION_UserForm.Equals(action))
            {
                log.Fine("Form:AD_For_ID=" + _node.GetVAF_Page_ID());
                return false;
            }
            /******	User Window					******/
            else if (MVAFWFlowNode.ACTION_UserWindow.Equals(action))
            {
            ...
            /********** SendSms ***********/
            else if (MVAFWFlowNode.ACTION_SMS.Equals(action))
            {
                GetPO(trx);
                if (_po == null)
                    throw new Exception("Persistent Object not found - VAF_TableView_ID="
            ...
            /************** FAX **********/
            else if (MVAFWFlowNode.ACTION_FaxEMail.Equals(action))
            {
                //log.Fine("EMail:EMailRecipient=" + _node.GetEMailRecipient());
                GetPO(trx);
                if (_po == null)
            ...
            /************** EMail&FAX *********/
            else if (MVAFWFlowNode.ACTION_EMailPlusFaxEMail.Equals(action))
            {
                ///////////Email//////////
                log.Fine("EMail:EMailRecipient=" + _node.GetEMailRecipient());
                GetPO(trx);
            ...
            //Forward Document
            else if (MVAFWFlowNode.ACTION_ForwardDocument.Equals(action))
            {
...
";
            ...
            //Move documnet
            else if (MVAFWFlowNode.ACTION_MoveDocument.Equals(action))
            {
            ...
            //Access Document
            else if (MVAFWFlowNode.ACTION_AccessDocument.Equals(action))
            {
                string res = string.Empty;
                GetPO(Get_TrxName());
                DocumentAction docAction = new DocumentAction();
...
";
        ...
        public string GetMailMessage(string message)
        {
            ...
            {
                MVAFWFlowTask lastAct = new MVAFWFlowTask(GetCtx(), GetLastActivity(), Get_TrxName());
        ...
        private void SendNoticeEMail(Trx trx)
        {
            ...
            if (re != null) //create report
            {
                ...
                log.Log(Level.INFO, "MVAFWFlowTask=>BI Report Byte Generated");
            }




            MVAFWFlowIncharge resp = GetResponsible(); //Get WF responsible
            if (resp.IsInvoker())
            {
            ...
            {
                MVAFRole role = resp.GetRole();
                if (role != null)
                {
                    MVAFUserContact[] users = MVAFUserContact.GetWithRole(role);
            ...
            {
                MVAFOrgDetail org = MVAFOrgDetail.Get(GetCtx(), _po.GetVAF_Org_ID(), null);
            ...
            SendEMail(client, 0, nodeEmails, subject, message, pdf, isHTML, 0, 0, MVAFWFlowNode.ACTION_AppsReport, report);
            ...
            {
                // Notice 
                MVAFUserContact user = MVAFUserContact.Get(p_ctx, userIds[i]);
                if (user.IsActive())
                {
                    if (user.IsNotificationNote() || user.GetNotificationType() == MVAFUserContact.NOTIFICATIONTYPE_EMailPlusNotice)
                    {
                        ...
                        if (report != null)
                        {
                            MVAFAttachment attachment = new MVAFAttachment(GetCtx(), MVAFNotice.Table_ID, note.GetVAF_Notice_ID(), trx);
                            if (isDocxFile)
                            {
                    ...
                    SendEMail(client, userIds[i], null, subject, message, pdf, isHTML, 0, 0, MVAFWFlowNode.ACTION_AppsReport, report);
                }
                else
                {
            ...
            String textMsg, int VAF_Screen_ID = 0) //throws Exception
        {
            ...
            if (GetNode().IsUserApproval() && (GetPO().GetType() == typeof(DocAction) || GetPO().GetType().GetInterface("DocAction") == typeof(DocAction)))
            {
                ...
                MVAFRole[] roles = user.GetRoles(_po.GetVAF_Org_ID());
        ...
        public bool ForwardTo(int VAF_UserContact_ID, String textMsg, bool isFromActivityForm = false, bool isForwarded = false)
        {
            ...
            //
            MVAFUserContact oldUser = MVAFUserContact.Get(GetCtx(), GetVAF_UserContact_ID());
            // Set current user on event and activity in case of forward
            if (isForwarded)
            {
                oldUser = MVAFUserContact.Get(GetCtx(), GetCtx().GetVAF_UserContact_ID());
            }
            MVAFUserContact user = MVAFUserContact.Get(GetCtx(), VAF_UserContact_ID);
        ...
        /// <param name="trx">transaction</param>
        private void FillParameter(MVAFJInstance pInstance, Trx trx)
        {
            GetPO(trx);
            //
            MVAFWFlowNodePara[] nParams = _node.GetParameters();
            ...
            for (int pi = 0; pi < iParams.Length; pi++)
            {
                ...
                {
                    MVAFWFlowNodePara nPara = nParams[np];
        ...
        private void SendEMail(string action)
        {
            ...
            else if (recipient.Equals(MVAFWFlowNode.EMAILRECIPIENT_DocumentBusinessPartner))
            {
            ...
            else if (recipient.Equals(MVAFWFlowNode.EMAILRECIPIENT_DocumentUser)) // new Addition
            {
            ...
            else if (recipient.Equals(MVAFWFlowNode.EMAILRECIPIENT_DocumentOwner))
            {
                if (doc != null)
                {
                    SendEMail(client, doc.GetDoc_User_ID(), null, subject, message, pdf, isHTML, tableID, RecID, action);
            ...
            else if (recipient.Equals(MVAFWFlowNode.EMAILRECIPIENT_WFResponsible))
            {
                MVAFWFlowIncharge resp = GetResponsible();
                if (resp.IsInvoker())
            ...
            else if (recipient.Equals(MVAFWFlowNode.EMAILRECIPIENT_DocumentOwnerSSupervisor))
            {
                if (isPOAsDocAction)
                {
            ...
            if (recipentUsers != null && recipentUsers.Count > 0 && action.Equals(MVAFWFlowNode.ACTION_EMailPlusFaxEMail))
            {
            ...
            String message, FileInfo pdf, bool isHTML, int VAF_TableView_ID, int record_ID, string action, byte[] bArray = null)
        {
            ...
            if (VAF_UserContact_ID != 0)
            {
                ...
                MVAFRole[] rl = user.GetRoles(GetVAF_Org_ID());
                // check applied if user has Org Access then only proceed to send
                // mail or notice
                if (rl.Length <= 0)
                    return;
                //Notice
                if (action != null && action.Equals(MVAFWFlowNode.ACTION_EMailPlusFaxEMail) &&
                  (MVAFUserContact.NOTIFICATIONTYPE_Notice.Equals(user.GetNotificationType()) ||
                   MVAFUserContact.NOTIFICATIONTYPE_EMailPlusNotice.Equals(user.GetNotificationType()) ||
                ...
                else if (!(MVAFUserContact.NOTIFICATIONTYPE_EMail.Equals(user.GetNotificationType()) ||
                    MVAFUserContact.NOTIFICATIONTYPE_EMailPlusFaxEMail.Equals(user.GetNotificationType()) ||
                    MVAFUserContact.NOTIFICATIONTYPE_EMailPlusNotice.Equals(user.GetNotificationType())))
                {
                    return;
        ...
        {
            if ((MVAFWFlowNode.ACTION_EMail.Equals(_node.GetAction()) || MVAFWFlowNode.ACTION_EMailPlusFaxEMail.Equals(_node.GetAction())) && _node != null && _node.IsBackground())
            {
                // Ctx
        ...
        {
            //	Create Notice
            MVAFNotice note = new MVAFNotice(GetCtx(), 0, Get_TrxName());
        ...
        public String GetHistoryHTML()
        {
            ...
            {
                MVAFWFlowEventLog audit = events[i];
        ...
        {
            StringBuilder sb = new StringBuilder("MVAFWFlowTask[");
            ...
            String message, FileInfo pdf, bool isHTML)
        {
            ...
            if (VAF_UserContact_ID != 0)
            {
                ...
                //email = user.GetEMail();
                if (!(MVAFUserContact.NOTIFICATIONTYPE_FaxEMail.Equals(user.GetNotificationType()) ||
                    MVAFUserContact.NOTIFICATIONTYPE_EMailPlusFaxEMail.Equals(user.GetNotificationType())))
                {
                    return;
                }
        ...
        {
            MVAFWFlowIncharge resp = new MVAFWFlowIncharge(GetCtx(), VAF_WFlow_Incharge_ID, Get_TrxName());
            if (resp.GetResponsibleType().Equals(MVAFWFlowIncharge.RESPONSIBLETYPE_Human))
            {
                return resp.GetVAF_UserContact_ID();
            }
            else if (resp.GetResponsibleType().Equals(MVAFWFlowIncharge.RESPONSIBLETYPE_Organization))
            {
                return (new MVAFOrgDetail(GetCtx(), Util.GetValueOfInt(po.Get_Value("VAF_Org_ID")), Get_TrxName()).GetSupervisor_ID());
            }
            else if (resp.GetResponsibleType().Equals(MVAFWFlowIncharge.RESPONSIBLETYPE_Role))
            {
            ...
            else if (resp.GetResponsibleType().Equals(MVAFWFlowIncharge.RESPONSIBLETYPE_SQL))
            {
        ...
        public String GetSummary()
        {
            ...
            MVAFWFlowNode node = MVAFWFlowNode.Get(po.GetCtx(), GetVAF_WFlow_Node_ID());
            ...
            else
            {
                ...
                if (sr != null)
                {
                    MVAFUserContact user = MVAFUserContact.Get(GetCtx(), sr.Value);
                    if (user != null)
