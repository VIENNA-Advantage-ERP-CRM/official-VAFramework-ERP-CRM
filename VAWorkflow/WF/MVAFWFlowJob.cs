...
{
    public class MVAFWFlowJob : X_VAF_WFlow_Job
    {
        /**	State Machine				*/
        ...
        /// <param name="VAF_WFlow_Job_ID">process id</param>
        /// <param name="trxName">transaction</param>
        public MVAFWFlowJob(Ctx ctx, int VAF_WFlow_Job_ID, Trx trxName)
            : base(ctx, VAF_WFlow_Job_ID, trxName)
        {
        ...
        /// <param name="trxName">transaction</param>
        public MVAFWFlowJob(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
            _state = new StateEngine(GetWFState());
            _state.SetCtx(GetCtx());
        ...
        public MVAFWFlowJob (MVAFWorkflow wf, ProcessInfo pi)
        ...
        public MVAFWFlowTask[] GetActivities(bool requery, bool onlyActive)
        {
            ...
            String sql = "SELECT * FROM VAF_WFlow_Task WHERE VAF_WFlow_Job_ID=" + GetVAF_WFlow_Job_ID() + "";
            if (onlyActive)
                sql += " AND Processed='N'";
            try
            {
        ...
        {
            StringBuilder sb = new StringBuilder("MVAFWFlowJob[")
                .Append(Get_ID())
