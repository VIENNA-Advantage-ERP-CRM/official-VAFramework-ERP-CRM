            ...
            if (values.Length > 0)
            {
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    if (!string.IsNullOrEmpty(values[i]))
                    {
                        ...
                        {
                            result1 = MReportTree.GetWhereClause(_ctx, _VAPA_FinancialReportingOrder_ID, MVABAccountBookElement.ELEMENTTYPE_Organization, Convert.ToInt32(values[i]));
                        }
                        ...
                        {
                            result1 = MReportTree.GetWhereClause(_ctx, _VAPA_FinancialReportingOrder_ID, MVABAccountBookElement.ELEMENTTYPE_Project, Convert.ToInt32(values[i]));
                        }
                        else if (columnName.Equals("VAF_OrgTrx_ID", StringComparison.OrdinalIgnoreCase))
                        {
                            result1 = MReportTree.GetWhereClause(_ctx, _VAPA_FinancialReportingOrder_ID, MVABAccountBookElement.ELEMENTTYPE_OrgTrx, Convert.ToInt32(values[i]));
                        }
                        else if (columnName.Equals("Account_ID", StringComparison.OrdinalIgnoreCase))
                        {
                            result1 = MReportTree.GetWhereClause(_ctx, _VAPA_FinancialReportingOrder_ID, MVABAccountBookElement.ELEMENTTYPE_Account, Convert.ToInt32(values[i]));
                        }
                        else if (columnName.Equals("VAB_Promotion_ID", StringComparison.OrdinalIgnoreCase))
                        {
                            result1 = MReportTree.GetWhereClause(_ctx, _VAPA_FinancialReportingOrder_ID, MVABAccountBookElement.ELEMENTTYPE_Campaign, Convert.ToInt32(values[i]));
                        }
