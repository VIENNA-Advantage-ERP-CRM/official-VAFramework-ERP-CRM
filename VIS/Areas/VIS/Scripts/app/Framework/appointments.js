; (function (VIS, $) {
    function AppointmentsForm() {
        ...
        */
        function initAppointments(VAF_TableView_ID, Record_ID, s_Ad_User_ID, s_User_Name, isTask, IsEmpployee) {
            if (window.WSP) {
                var divaptbusy = $("<div id='divAptBusy' class='wsp-busy-indicater'></div>");
                $("body").append(divaptbusy);
                if (isTask == false) {
                    divaptbusy.show();
                    WSP.AppointmentsForm.init(VAF_TableView_ID, Record_ID, s_Ad_User_ID, s_User_Name, divaptbusy, IsEmpployee);
                }
                else {
                    divaptbusy.show();
                    WSP.TaskForm.init(VAF_TableView_ID, Record_ID, s_Ad_User_ID, s_User_Name, divaptbusy, IsEmpployee);
                }
