            ...
            if (C_Period_ID != 0)
            {
                ...
                try
                {
                    ...
                    while (idr.Read())
                    {
                        ...
                        //sql = "select SUM(nvl(fl.qtyentered,0)) from VAB_Forecastline fl inner join VAB_Forecast f on (f.VAB_Forecast_id = fl.VAB_Forecast_id) where f.vaf_client_id = " + mf.GetVAF_Client_ID() + " and fl.m_product_id =  " + Util.GetValueOfInt(idr[0]) + " and fl.Processed = 'Y' and fl.isactive = 'Y' and f.c_period_id = " + C_Period_ID;
                      ...
                      //  sql = "select SUM(nvl(fl.pricestd,0)) from VAB_Forecastline fl inner join VAB_Forecast f on (f.VAB_Forecast_id = fl.VAB_Forecast_id) where f.vaf_client_id = " + mf.GetVAF_Client_ID() + " and fl.m_product_id =  " + Util.GetValueOfInt(idr[0]) + " and fl.Processed = 'Y' and fl.isactive = 'Y' and f.c_period_id = " + C_Period_ID;
