...
namespace VAdvantage.Model
{
    ...
    public class SubTableUtil
    {
        ...
        private bool GenerateTable()
        {
            ...
            String catalog = "";
            String schema = BaseLibrary.DataBase.DB.GetSchema();
            ...
            for (int rscount = 0; rscount <= rs.Tables[0].Rows.Count - 1; rscount++)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    if (columnName.Equals(dCol.GetColumnName(), StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        BaseLibrary.DataBase.DB.ExecuteUpdateMultiple(sql, false, null);
                        found = true;
                        list.Remove(list[i]);
                        break;
                    }
                ...
                if (!found)
                {
                    ...
                    BaseLibrary.DataBase.DB.ExecuteQuery(sql, null);
                }
            }
            //rs.close();
            trx.Close();
            ...
            if (noColumns == 0)
            {
                ...
                return BaseLibrary.DataBase.DB.ExecuteUpdateMultiple(sql, false, null) >= 0;
            }
            //	New Columns
            for (int i = 0; i < list.Count(); i++)
            {
        ...
        private static bool CreateColumn(MColumn col, MTable table, bool alsoInDB)
        {
            ...
            bool success = BaseLibrary.DataBase.DB.ExecuteUpdateMultiple(sql, false, table.Get_TrxName()) >= 0;
            if (success)
