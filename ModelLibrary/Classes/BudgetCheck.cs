            ...
            DateTime? date, List<BudgetControl> _listBudgetControl, Trx trxName, char order, int OrderId)
        {
            ...
            if (dsBudgetControlAmount != null && dsBudgetControlAmount.Tables.Count > 0)
            {
                for (int i = 0; i < dsBudgetControlAmount.Tables[0].Rows.Count; i++)
                {
                    ...
                    budgetControl.VAB_AddressFrom_ID = dsBudgetControlAmount.Tables[0].Columns.Contains("VAB_LocFrom_ID") ? Util.GetValueOfInt(dsBudgetControlAmount.Tables[0].Rows[i]["VAB_LocFrom_ID"]) : 0;
                    budgetControl.VAB_AddressTo_ID = dsBudgetControlAmount.Tables[0].Columns.Contains("VAB_LocTo_ID") ? Util.GetValueOfInt(dsBudgetControlAmount.Tables[0].Rows[i]["VAB_LocTo_ID"]) : 0;
