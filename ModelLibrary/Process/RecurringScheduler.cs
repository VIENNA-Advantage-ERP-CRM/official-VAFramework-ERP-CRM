    ...
    public class RecurringScheduler : SvrProcess
    {
        ...
        protected override string DoIt()
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    Recurring = new MVABRecurring(GetCtx(), Util.GetValueOfInt(ds.Tables[0].Rows[i]["VAB_Recurring_ID"]), Get_Trx());
                    dateDoc = Recurring.GetDateNextRun();
                    if (dateDoc != null && Recurring.CalculateRuns())
                    {
                        //  throw new Exception("No Runs Left");
                        MVABRecurringRun run = new MVABRecurringRun(GetCtx(), Recurring);
                        String msg = "@Created@ ";
                        // if (Recurring.GetDateNextRun() == DateTime.Now.Date || Recurring.GetDateNextRun() == null)
                        //{
                        //	Copy
                        if (Recurring.GetRecurringType().Equals(MVABRecurring.RECURRINGTYPE_Order))
                        {
                        ...
                        else if (Recurring.GetRecurringType().Equals(MVABRecurring.RECURRINGTYPE_Invoice))
                        {
                        ...
                        else if (Recurring.GetRecurringType().Equals(MVABRecurring.RECURRINGTYPE_Project))
                        {
                        ...
                        else if (Recurring.GetRecurringType().Equals(MVABRecurring.RECURRINGTYPE_Payment))
                        {
