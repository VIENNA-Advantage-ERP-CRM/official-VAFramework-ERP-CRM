...
namespace ViennaAdvantage.Process
{
    public class BalanceReport : SvrProcess
    {
     ...
     //   int _NoOfRecord = 0;
        ...
        private static String _Insert = @"INSERT INTO  T_BALSHEET "
             + "( parent_id, AccountType,C_PROJECT_ID,node_id,VAF_Client_ID, VAF_Org_ID,"
    ...
    a.parent_id,max(c.issummary) as IsSummary,c.isintermediatecode, a.node_id,c.vaf_client_id,c.vaf_org_id,c.value, c.name,SUM(f.amtacctdr) AS PrMonDb,SUM(f.amtacctcr) AS PrMonCr, CASE WHEN c.AccountType='A'  THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Total");
            ...
            Createsql(false);
            _msql.Append(m_parameterWhere);
            _msql.Append(" t2  ON (t1.value=t2.value) where t1.Total>0 group by t1.parent_id, t1.AccountType,  t1.C_PROJECT_ID,   t1.node_id,  t1.vaf_client_id,  t1.vaf_org_id,  t1.value     ,  t1.name  ,  t1.IsSummary,t1.PrMonDb,  t1.PrMonCr,  t2.CrMonDb,  t2.CrMonCr,t1.isintermediatecode");
            //_msql.Append(" ");
            //_msql.Append(sql);
            //_msql.Append(sql);
            ...
            if (_LedgerType == "N" || _LedgerType == "A")
            {
                ...
                if (dsrecord.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < dsrecord.Tables[0].Rows.Count; i++)
                    {
                        ...
                        sqlnew = "select GL_budget_id from fact_acct_balance where vaf_client_id=" + GetCtx().GetVAF_Client_ID() + " and account_id=" + _Node_id + " and GL_budget_id is not null";
                        GL_budget_id = Util.GetValueOfInt(DB.ExecuteScalar(sqlnew, null, null));
                        sqlnew = "select sum(amtacctdr) as Debit,sum(amtacctcr) as Credit from fact_acct_balance where vaf_client_id=" + GetCtx().GetVAF_Client_ID() + " and account_id=" + _Node_id + " and GL_budget_id is not null";
                        ...
                        _sqlUpdate = "Update T_BALSHEET set GL_budget_id=" + GL_budget_id + ",BUDGETVAL=" + _Gl_budgetvalue + ",VARINPERCENT=" + _totalBudgetValue + "  where vaf_client_id=" + GetCtx().GetVAF_Client_ID() + " and node_id=" + _Node_id;
        ...
        private void GroupBy(bool totalQty)
        {
            if (totalQty)
            {
                m_parameterWhere.Append(@" GROUP BY a.parent_id, c.AccountType,a.node_id,c.value,  c.name,c.AccountType,c.vaf_client_id,c.vaf_org_id,c.AccountType,f.C_PROJECT_ID,c.isintermediatecode)abc where abc.total!=0)");
            }
            else
            {
                m_parameterWhere.Append(@" GROUP BY a.parent_id, c.AccountType,a.node_id,c.value,  c.name,c.AccountType,c.vaf_client_id,c.vaf_org_id,c.AccountType,f.C_PROJECT_ID,c.isintermediatecode)abc where abc.Currentamount!=0)");
            }
        }

        private void CreateView()
        {
            string _sqlview = @"CREATE OR REPLACE FORCE VIEW BALSHEET_V  as SELECT vaf_client_id            ,  accounttype                   ,  vaf_org_id                     ,  Rownum AS BalSheet_V_ID       ,  C_PROJECT_ID                  ,  C_PERIOD_ID                   ,  ledgertype              AS LedgerType      ,  LEDGERNAME              AS LEDGER          ,  issummary               AS ShowSummaryLevel,  NVL(ABS(PMDEBIT),0)     AS PreMonDebit     ,  NVL(ABS(PMCREDIT),0)    AS PreMonCredit    ,  NVL(ABS(CMDEBIT),0)     AS CRMonDebit      ,  NVL(ABS(CMCREDIT),0)    AS CRMonCredit     ,  NVL(ABS(TOTALDEBIT),0)  AS TotalDebit      ,  NVL(ABS(TOTALCREDIT),0) AS TotalCredit     ,  NVL(ABS(DIFFDEBIT),0)   AS DiffDebitVal    ,  NVL(ABS(DIFFCREDIT),0)  AS DiffCreditVal   ,  ledgercode                                 ,  SUBSTR( ledgercode,1,1)  AS ledgercode1     ,  SUBSTR( ledgercode,2,2)  AS ledgercode2     ,  SUBSTR( ledgercode,4,2)  AS ledgercode3     ,  SUBSTR( ledgercode,6,2)  AS ledgercode4     ,  SUBSTR( ledgercode,8,2)  AS ledgercode5     ,  SUBSTR( ledgercode,10,2) AS ledgercode6     ,  SUBSTR( ledgercode,12,2) AS ledgercode7   FROM t_Balsheet  WHERE accounttype ='A' OR accounttype      ='L' OR accounttype      ='O' ORDER BY LEDGERCODE";
            DB.ExecuteQuery(_sqlview, null, null);
        }
        private void CreateViewAsTree()
        {
            string _sqlview = @"CREATE OR REPLACE FORCE VIEW BALSHEET_V AS  SELECT vaf_client_id      ,  accounttype             ,  vaf_org_id               ,  Rownum AS BalSheet_V_ID ,  C_PROJECT_ID            ,  C_PERIOD_ID             ,  ledgertype AS LedgerType,  '.'  || LPAD (' ', LEVEL * 3)  || LEDGERNAME           AS LEDGER          ,  issummary               AS ShowSummaryLevel,  NVL(ABS(PMDEBIT),0)     AS PreMonDebit     ,  NVL(ABS(PMCREDIT),0)    AS PreMonCredit    ,  NVL(ABS(CMDEBIT),0)     AS CRMonDebit      ,  NVL(ABS(CMCREDIT),0)    AS CRMonCredit     ,  NVL(ABS(TOTALDEBIT),0)  AS TotalDebit      ,  NVL(ABS(TOTALCREDIT),0) AS TotalCredit     ,  NVL(ABS(DIFFDEBIT),0)   AS DiffDebitVal    ,  NVL(ABS(DIFFCREDIT),0)  AS DiffCreditVal   ,  ledgercode                                 ,  SUBSTR( ledgercode,1,1)  AS ledgercode1     ,  SUBSTR( ledgercode,2,2)  AS ledgercode2     ,  SUBSTR( ledgercode,4,2)  AS ledgercode3     ,  SUBSTR( ledgercode,6,2)  AS ledgercode4     ,  SUBSTR( ledgercode,8,2)  AS ledgercode5     ,  SUBSTR( ledgercode,10,2) AS ledgercode6     ,  SUBSTR( ledgercode,12,2) AS ledgercode7   FROM t_Balsheet  WHERE accounttype ='A' OR accounttype      ='L' OR accounttype      ='O' START WITH parent_id      =0 CONNECT BY parent_id = PRIOR node_id ORDER SIBLINGS BY LEDGERCODE";
            DB.ExecuteQuery(_sqlview, null, null);
        }
