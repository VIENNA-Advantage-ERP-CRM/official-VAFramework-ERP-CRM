...
namespace ViennaAdvantage.Process
{
    ...
    a.parent_id,max(c.issummary) as IsSummary,c.isintermediatecode, a.node_id,c.vaf_client_id,c.vaf_org_id,c.value, c.name,SUM(f.amtacctdr) AS PrMonDb,SUM(f.amtacctcr) AS PrMonCr, CASE WHEN c.AccountType='A'  THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Total");
            ...
            _m_CaseSql.Append(@"(SELECT * From (Select distinct  c.AccountType,f.VAB_PROJECT_ID,
               a.parent_id,max(c.issummary) AS IsSummary,c.isintermediatecode, a.node_id,c.vaf_client_id,c.vaf_org_id,c.value, c.name,SUM(f.amtacctdr) AS CrMonDb,SUM(f.amtacctcr) AS CrMonCr, CASE WHEN c.AccountType='A'  THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Currentamount");
            _msql.Append(_m_CaseSql);
            _msql.Append(sql);
            ...
            Createsql(false);
            _msql.Append(m_parameterWhere);
            _msql.Append(" t2  ON (t1.value=t2.value) where t1.Total>0 group by t1.parent_id, t1.AccountType,  t1.VAB_PROJECT_ID,   t1.node_id,  t1.vaf_client_id,  t1.vaf_org_id,  t1.value     ,  t1.name  ,  t1.IsSummary,t1.PrMonDb,  t1.PrMonCr,  t2.CrMonDb,  t2.CrMonCr,t1.isintermediatecode");
            //_msql.Append(" ");
            //_msql.Append(sql);
            //_msql.Append(sql);
        ...
        private void Createsql(bool totalQty)
        {
            ...
            {
                _LedgerType = "A";
                m_parameterWhere.Append(" ) WHERE c.vaf_client_id=" + GetCtx().GetVAF_Client_ID() + " and c.AccountType='A' or c.AccountType='L' or c.AccountType='O' ");
            }
            else
            {
                if (_LedgerType == "N")
