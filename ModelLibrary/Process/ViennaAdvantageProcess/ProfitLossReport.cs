      ...
      //  private int _VAB_LocTo_ID = 0;
       ...
       // int _NoOfRecord = 0;
        ...
        protected override string DoIt()
        {
            ...
            _m_OuterSql.Append(@"SELECT distinct t1.parent_id,t1.AccountType,t1.VAB_PROJECT_ID,
    t1.node_id,t1.vaf_client_id,t1.vaf_org_id,t1.value AS value,t1.name as name,nvl(max(t2.Currentamount),0) AS Currentamount,(nvl(max(t1.Total),0)-nvl(max(t2.Currentamount),0)) AS Previous,
            ...
            _m_CaseSql.Append(@"(SELECT * from(Select distinct    c.AccountType,f.VAB_PROJECT_ID,
         a.parent_id,max(c.issummary) as IsSummary,c.isintermediatecode, a.node_id,c.vaf_client_id,c.vaf_org_id,c.value, c.name, CASE WHEN c.AccountType='E' THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Total");
            ...
            _m_CaseSql.Append(@"(SELECT * From (Select distinct  c.AccountType,f.VAB_PROJECT_ID,
               a.parent_id,max(c.issummary) AS IsSummary,c.isintermediatecode, a.node_id,c.vaf_client_id,c.vaf_org_id,c.value, c.name, CASE WHEN c.AccountType='E'  THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Currentamount");
            _msql.Append(_m_CaseSql);
            _msql.Append(sql);
            ...
            Createsql(false);
            _msql.Append(m_parameterWhere);
            _msql.Append(" t2  ON (t1.value=t2.value) where t1.Total>0 group by t1.parent_id, t1.AccountType,  t1.VAB_PROJECT_ID,   t1.node_id,  t1.vaf_client_id,  t1.vaf_org_id,  t1.value     ,  t1.name  ,  t1.IsSummary,t1.isintermediatecode");
            //_msql.Append(" ");
            //_msql.Append(sql);
            //_msql.Append(sql);
            ...
            //            t1.VAB_SalesRegionState_ID,t1.VAB_Promotion_ID,t1.VAB_LocFrom_ID,t1.VAB_LocTo_ID  ,t1.User1_ID,t1.User2_ID,t1.UserElement1_ID,t1.UserElement2_ID,t1.ACCOUNTTYPE
            ...
            //      f.VAB_SalesRegionState_ID,f.VAB_Promotion_ID,f.VAB_LocFrom_ID,f.VAB_LocTo_ID  ,f.User1_ID,f.User2_ID,f.UserElement1_ID,f.UserElement2_ID, c.name,CASE WHEN c.AccountType='A' OR c.AccountType  ='E' THEN (SUM(f.amtacctdr)-SUM(f.amtacctcr))
            ...
            //            f.VAB_SalesRegionState_ID,f.VAB_Promotion_ID,f.VAB_LocFrom_ID,f.VAB_LocTo_ID  ,f.User1_ID,f.User2_ID,f.UserElement1_ID,f.UserElement2_ID,g.VAGL_Budget_ID) t1 inner JOIN (SELECT g.VAGL_Budget_ID,a.parent_id,a.node_id,c.value AS value,c.name, cASE WHEN c.AccountType='A' OR c.AccountType  ='E' THEN (SUM(f.amtacctdr)-SUM(f.amtacctcr))
            ...
            //      f.VAB_SalesRegionState_ID,f.VAB_Promotion_ID,f.VAB_LocFrom_ID,f.VAB_LocTo_ID  ,f.User1_ID,f.User2_ID,f.UserElement1_ID,f.UserElement2_ID FROM VAF_TreeInfoChild a INNER JOIN VAB_Acct_Element c ON c.VAB_Acct_Element_id = a.node_id LEFT OUTER JOIN Actual_Acct_Balance f
            ...
            //      f.VAB_SalesRegionState_ID,f.VAB_Promotion_ID,f.VAB_LocFrom_ID,f.VAB_LocTo_ID  ,f.User1_ID,f.User2_ID,f.UserElement1_ID,f.UserElement2_ID,a.parent_id,a.node_id,g.VAGL_Budget_ID) t2 ON (t1.value=t2.value) ");
        ...
        private void Createsql(bool totalQty)
        {
            ...
            //}
            //if (_VAB_LocFrom_ID != 0)
            //{
            //    m_parameterWhere.Append(" and VAB_LocFrom_ID=" + _VAB_LocFrom_ID + "");
            //}
            //if (_VAB_LocTo_ID != 0)
            //{
            //    m_parameterWhere.Append(" and VAB_LocTo_ID=" + _VAB_LocTo_ID + "");
            //}
            //if (_VAB_LocFrom_ID != 0)
            //{
            //    m_parameterWhere.Append(" and VAB_LocFrom_ID=" + _VAB_LocFrom_ID + "");
            ...
            {
                _LedgerType = "A";
                m_parameterWhere.Append(" ) WHERE c.vaf_client_id=" + GetCtx().GetVAF_Client_ID() + " and c.AccountType='E' or c.AccountType='R'  ");
            }
            else
            {
                if (_LedgerType == "N")
        ...
        private void GroupBy(bool totalQty)
        {
            if (totalQty)
            {
                m_parameterWhere.Append(@" GROUP BY a.parent_id, c.AccountType,a.node_id,c.value,  c.name,c.AccountType,c.vaf_client_id,c.vaf_org_id,c.AccountType,f.VAB_PROJECT_ID,c.isintermediatecode)abc where abc.total!=0 )");

            }
            else
            {
                m_parameterWhere.Append(@" GROUP BY a.parent_id, c.AccountType,a.node_id,c.value,  c.name,c.AccountType,c.vaf_client_id,c.vaf_org_id,c.AccountType,f.VAB_PROJECT_ID,c.isintermediatecode)abc where abc.Currentamount!=0 )");
            }
        }

        private void CreateView()
        {
            string _sqlview = @"CREATE OR REPLACE FORCE VIEW PROFITLOSS_V  as SELECT vaf_client_id            ,  accounttype                   ,  vaf_org_id                     ,  rownum AS ProfitLoss_V_ID     ,  VAB_PROJECT_ID                  ,  VAB_YEARPERIOD_ID                   ,  ledgertype               AS LedgerType      ,  LEDGERNAME               AS LEDGER          ,  issummary                AS ShowSummaryLevel,  NVL(ABS(budgetval),0)    AS budgetval       ,  NVL(ABS(varinpercent),0) AS varinpercent    ,  NVL(ABS(crmonval),0)     AS crmonval        ,  NVL(ABS(tpmonvalue),0)   AS tpmonvalue      ,  NVL(ABS(totalvalue),0)   AS totalvalue      ,  ledgercode                                  ,  SUBSTR( ledgercode,1,1)  AS ledgercode1      ,  SUBSTR( ledgercode,2,2)  AS ledgercode2      ,  SUBSTR( ledgercode,4,2)  AS ledgercode3      ,  SUBSTR( ledgercode,6,2)  AS ledgercode4      ,  SUBSTR( ledgercode,8,2)  AS ledgercode5      ,  SUBSTR( ledgercode,10,2) AS ledgercode6      ,  SUBSTR( ledgercode,12,2) AS ledgercode7   FROM VAT_BALSHEET  WHERE accounttype ='E' OR accounttype      ='R' ORDER BY LEDGERCODE";
            DB.ExecuteQuery(_sqlview, null, null);
        }
        private void CreateViewAsTree()
        {
            string _sqlview = @"CREATE OR REPLACE FORCE VIEW PROFITLOSS_V AS  SELECT vaf_client_id       ,  accounttype              ,  vaf_org_id                ,  rownum AS ProfitLoss_V_ID,  VAB_PROJECT_ID             ,  VAB_YEARPERIOD_ID              ,  ledgertype AS LedgerType ,  '.'  || LPAD (' ', LEVEL * 3)  || LEDGERNAME            AS LEDGER          ,  issummary                AS ShowSummaryLevel,  NVL(ABS(budgetval),0)    AS budgetval       ,  NVL(ABS(varinpercent),0) AS varinpercent    ,  NVL(ABS(crmonval),0)     AS crmonval        ,  NVL(ABS(tpmonvalue),0)   AS tpmonvalue      ,  NVL(ABS(totalvalue),0)   AS totalvalue      ,  ledgercode                                  ,  SUBSTR( ledgercode,1,1)  AS ledgercode1      ,  SUBSTR( ledgercode,2,2)  AS ledgercode2      ,  SUBSTR( ledgercode,4,2)  AS ledgercode3      ,  SUBSTR( ledgercode,6,2)  AS ledgercode4      ,  SUBSTR( ledgercode,8,2)  AS ledgercode5      ,  SUBSTR( ledgercode,10,2) AS ledgercode6      ,  SUBSTR( ledgercode,12,2) AS ledgercode7   FROM VAT_BALSHEET  WHERE accounttype ='E' OR accounttype      ='R' START WITH parent_id      =0 CONNECT BY parent_id = PRIOR node_id ORDER SIBLINGS BY LEDGERCODE";
            DB.ExecuteQuery(_sqlview, null, null);
        }
        private void CreateBalanceLine()
        {
            ...
            if (_VAB_YearPeriod_ID != 0 )
            {
                sql.Append("and e.VAB_YearPeriod_ID=" + _VAB_YearPeriod_ID + "");
            }
            if (_LedgerType != "")
            {
            ...
            //    _VAB_LocFrom_ID == 0 && _VAB_LocTo_ID == 0 && _VAB_SalesRegionState_ID == 0 && _VAB_BillingCode_ID == 0 && _VAB_Promotion_ID == 0 && _VAB_Project_ID == 0 && _VAB_BusinessPartner_ID == 0 &&
        ...
        private void WhereClause()
        {
            ...
            if (_VAB_YearPeriod_ID != 0 )
            {
                sql.Append(" and _VAB_YearPeriod_ID=" + _VAB_YearPeriod_ID + "");
            }
            if (_LedgerType != "")
            {
