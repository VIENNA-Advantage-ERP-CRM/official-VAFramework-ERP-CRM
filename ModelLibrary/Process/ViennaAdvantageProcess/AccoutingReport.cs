...
namespace ViennaAdvantage.Process
{
    public class AccoutingReport : SvrProcess
    {
       ...
       // int _NoOfRecord = 0;
        ...
        private static String _Insert = @"INSERT INTO  T_Statement "
             + "( parent_id, AccountType,VAM_Product_ID,VAB_BUSINESSPARTNER_ID,VAB_PROMOTION_ID,VAB_PROJECT_ID,VAB_BILLINGCODE_ID,VAB_SALESREGIONSTATE_ID,VAB_LocFrom_ID,VAB_LocTo_ID,USER1_ID,USER2_ID,USERELEMENT1_ID,USERELEMENT2_ID,node_id,VAF_Client_ID, VAF_Org_ID,"
    ...
    t1.VAB_LocFrom_ID,t1.VAB_LocTo_ID,t1.USER1_ID,t1.USER2_ID,t1.USERELEMENT1_ID,t1.USERELEMENT2_ID,t1.node_id,t1.vaf_client_id,t1.vaf_org_id,t1.value AS value,t1.name as name,nvl(max(t2.Currentamount),0) AS Currentamount,(nvl(max(t1.Total),0)-nvl(max(t2.Currentamount),0)) AS Previous,
         ...
         f.VAB_LocFrom_ID,f.VAB_LocTo_ID,f.USER1_ID,f.USER2_ID,f.USERELEMENT1_ID,f.USERELEMENT2_ID,a.parent_id,max(c.issummary) as IsSummary,c.isintermediatecode, a.node_id,c.vaf_client_id,c.vaf_org_id,c.value, c.name, CASE WHEN c.AccountType='A' OR c.AccountType  ='E' THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Total");
            ...
            Createsql(false);
            _msql.Append(m_parameterWhere);
            _msql.Append(" t2  ON (t1.value=t2.value) group by t1.parent_id, t1.AccountType,t1.VAM_Product_ID,  t1.VAB_BUSINESSPARTNER_ID,  t1.VAB_PROMOTION_ID,  t1.VAB_PROJECT_ID,  t1.VAB_BILLINGCODE_ID,  t1.VAB_SALESREGIONSTATE_ID,  t1.VAB_LocFrom_ID,  t1.VAB_LocTo_ID,  t1.USER1_ID,  t1.USER2_ID,  t1.USERELEMENT1_ID,  t1.USERELEMENT2_ID,  t1.node_id,  t1.vaf_client_id,  t1.vaf_org_id,  t1.value     ,  t1.name  ,  t1.IsSummary,t1.isintermediatecode");
            //_msql.Append(" ");
            //_msql.Append(sql);
            //_msql.Append(sql);
        ...
        private void GroupBy(bool totalQty)
        {
            if (totalQty)
            {
                m_parameterWhere.Append(@" GROUP BY a.parent_id, c.AccountType,a.node_id,c.value,  c.name,c.AccountType,c.vaf_client_id,c.vaf_org_id,c.AccountType,f.VAM_Product_ID,f.VAB_BUSINESSPARTNER_ID,f.VAB_PROMOTION_ID,f.VAB_PROJECT_ID,f.VAB_BILLINGCODE_ID,f.VAB_SALESREGIONSTATE_ID,
                               f.VAB_LocFrom_ID,f.VAB_LocTo_ID,f.USER1_ID,f.USER2_ID,f.USERELEMENT1_ID,f.USERELEMENT2_ID,c.isintermediatecode)abc where abc.total!=0)");
            }
            else
            {
                m_parameterWhere.Append(@" GROUP BY a.parent_id, c.AccountType,a.node_id,c.value,  c.name,c.AccountType,c.vaf_client_id,c.vaf_org_id,c.AccountType,f.VAM_Product_ID,f.VAB_BUSINESSPARTNER_ID,f.VAB_PROMOTION_ID,f.VAB_PROJECT_ID,f.VAB_BILLINGCODE_ID,f.VAB_SALESREGIONSTATE_ID,
                               f.VAB_LocFrom_ID,f.VAB_LocTo_ID,f.USER1_ID,f.USER2_ID,f.USERELEMENT1_ID,f.USERELEMENT2_ID,c.isintermediatecode)abc where abc.Currentamount!=0)");
            }
        }
        ...
        private void CreateBalanceLine()
        {
            ...
            if (_VAB_LocFrom_ID != 0)
            {
                sql.Append(" and e.VAB_LocFrom_ID=" + _VAB_LocFrom_ID + "");
            }
            if (_VAB_LocTo_ID != 0)
            {
                sql.Append(" and e.VAB_LocTo_ID=" + _VAB_LocTo_ID + "");
            }
            if (_VAB_LocFrom_ID != 0)
            {
                sql.Append(" and e.VAB_LocFrom_ID=" + _VAB_LocFrom_ID + "");
        ...
        private void WhereClause()
        {
            ...
            if (_VAB_LocFrom_ID != 0 )
            {
                sql.Append(" and VAB_LocFrom_ID=" + _VAB_LocFrom_ID + "");
            }
            if (_VAB_LocTo_ID != 0 )
            {
                sql.Append(" and VAB_LocTo_ID=" + _VAB_LocTo_ID + "");
            }
            if (_VAB_LocFrom_ID != 0)
            {
                sql.Append(" and VAB_LocFrom_ID=" + _VAB_LocFrom_ID + "");
        ...
        protected override void Prepare()
        {
            ...
            for (int i = 0; i < para.Length; i++)
            {
                ...
                else if (name.Equals("VAB_LocFrom_ID"))
                {
                    _VAB_LocFrom_ID = Util.GetValueOfInt(para[i].GetParameter());
                }
                else if (name.Equals("VAB_LocTo_ID"))
                {
                    _VAB_LocTo_ID = Util.GetValueOfInt(para[i].GetParameter());
                }
