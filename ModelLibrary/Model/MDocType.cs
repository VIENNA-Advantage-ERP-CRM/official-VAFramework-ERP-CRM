...
namespace VAdvantage.Model
{
    public class MDocType : X_C_DocType
    {
        ...
        public void SetGL_Category_ID()
        {
            ...
            int GL_Category_ID = BaseLibrary.DataBase.DB.GetSQLValue(Get_TrxName(), sql);
            if (GL_Category_ID == 0)
            {
                sql = "SELECT * FROM GL_Category WHERE AD_Client_ID=@param1";
                GL_Category_ID = BaseLibrary.DataBase.DB.GetSQLValue(Get_TrxName(), sql, GetAD_Client_ID());
            }
