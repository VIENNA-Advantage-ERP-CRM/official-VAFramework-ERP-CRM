...
{
    public class MVABBPartCategory : X_VAB_BPart_Category
    {
        /**
	     ...
	     *	@return MBPGroup
	     */
        public static MVABBPartCategory Get(Ctx ctx, int VAB_BPart_Category_ID)
        {
            int key = VAB_BPart_Category_ID;
            MVABBPartCategory retValue = (MVABBPartCategory)_cache[key];
            if (retValue != null)
                return retValue;
            retValue = new MVABBPartCategory(ctx, VAB_BPart_Category_ID, null);
         ...
         *	@return MBPGroup
         */
        public static MVABBPartCategory GetDefault(Ctx ctx)
        {
            int VAF_Client_ID = ctx.GetVAF_Client_ID();
            int key = VAF_Client_ID;
            MVABBPartCategory retValue = (MVABBPartCategory)_cacheDefault[key];
            if (retValue != null)
                return retValue;
            ...
            try
            {
                ...
                {
                    retValue = new MVABBPartCategory(ctx, dr, null);
                    if (retValue.Get_ID() != 0)
         ...
         *	@return MBPGroup
         */
        public static MVABBPartCategory GetOfBPartner(Ctx ctx, int VAB_BusinessPartner_ID)
        {
            MVABBPartCategory retValue = null;
            DataTable dt = null;
            String sql = "SELECT * FROM VAB_BPart_Category g "
        ...
        //	Cache						
        private static CCache<int, MVABBPartCategory> _cache
            = new CCache<int, MVABBPartCategory>("BP_Group", 10);
        //	Default Cache					
        private static CCache<int, MVABBPartCategory> _cacheDefault
            = new CCache<int, MVABBPartCategory>("BP_Group", 5);
        //Logger	
        private static VLogger _log = VLogger.GetVLogger(typeof(MVABBPartCategory).FullName);
         ...
         */
        public MVABBPartCategory(Ctx ctx, int VAB_BPart_Category_ID, Trx trxName) :
         ...
         *	@param trxName transaction
         */
        public MVABBPartCategory(Ctx ctx, DataRow dr, Trx trxName) :
            base(ctx, dr, trxName)
        {
