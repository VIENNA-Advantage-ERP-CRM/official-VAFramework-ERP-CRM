...
namespace VAdvantage.Model
{
    public class MCostQueue : X_M_CostQueue
    {
            ...
            try
            {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        {
                            decimal productCostsQty = MCostQueue.CheckQtyAvailablity(ctx, acctSchema, AD_Client_ID, AD_Org_ID, product, M_ASI_ID);
                        ...
                        if (windowName == "Material Receipt" || windowName == "Customer Return" || windowName == "Shipment" || windowName == "Return To Vendor")
                        {
                            ...
                            else
                            {
                                ...
                                price = MCostQueue.CalculateCost(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, AD_Org_ID);
                            ...
                            else if (!inoutline.IsCostCalculated())
                            {
                                ...
                                if (!client.IsCostImmediate())
                                {
                                    if (!CreateCostQueueForMatchPO(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, AD_Org_ID,
                                        Price, inoutline.GetMovementQty(), inoutline, trxName, optionalStrPO: optionalstr))
                                    {
                            ...
                            if (cdId <= 0)
                            {
                                if (cmPrice > 0)
                                {
                                    cdSourceWarehouse = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                                        inventoryLine, inoutline, movementline, invoiceline, po, 0, Decimal.Negate(cmPrice), Decimal.Negate(Qty), null, trxName);
                                }
                                else
                                {
                                    cdSourceWarehouse = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                                        inventoryLine, inoutline, movementline, invoiceline, po, 0, Decimal.Negate(Price), Decimal.Negate(Qty), null, trxName);
                                }
                            }
                            query.Clear();
                        ...
                        if (((windowName == "Invoice(Vendor)" && product == null) || (windowName == "Invoice(Vendor)" && product != null && product.GetProductType() == "E")))
                        {
                            ...
                            if (dsLandedCostAllocation != null && dsLandedCostAllocation.Tables.Count > 0 && dsLandedCostAllocation.Tables[0].Rows.Count > 0)
                            {
                                ...
                                for (int lca = 0; lca < dsLandedCostAllocation.Tables[0].Rows.Count; lca++)
                                {
                                    ...
                                    {
                                        query.Clear();
                                        query.Append("SELECT M_CostDetail_ID FROM M_CostDetail WHERE IsActive = 'Y' AND C_InvoiceLine_ID = " + invoiceline.GetC_InvoiceLine_ID());
                                        if (invoiceline.GetC_OrderLine_ID() > 0)
                                        {
                                    ...
                                    if (cd != null && ((optionalstr == "window" && client.IsCostImmediate()) || (optionalstr == "process")))
                                    {
                                        ...
                                        {
                                            cd.UpdateProductCost(windowName, cd, acctSchema, productLca, M_ASI_ID, invoiceline.GetAD_Org_ID(), optionalStrCd: optionalstr);
                                        }
                                        if (cd != null)
                                        {
                            ...
                            if (cd == null || cd.GetM_CostDetail_ID() <= 0)
                            {
                                if (cmPrice != 0)
                                {
                                    cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                                          inventoryLine, inoutline, movementline, invoiceline, po, 0, cmPrice, Qty, null, trxName);
                                }
                                else
                                {
                                    cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                                         inventoryLine, inoutline, movementline, invoiceline, po, 0, Price, Qty, null, trxName);
                                }
                            }
                        }
                                ...
                                ))
                        {
                            ...
                            else if (windowName == "Production Execution")
                            {
                                ...
                                {
                                    //1st entry either of FIFO of LIFO 
                                    updateCostQueue(product, M_ASI_ID, acctSchema, Util.GetValueOfInt(po.Get_Value("AD_Org_ID")), costElement, Decimal.Negate(Qty));
                ...
                {
                    #region Handle Primary accounting Schema when we can not find any Accounting Schema on Assigned Org
                    //                    sql = "SELECT C_AcctSchema1_ID FROM AD_ClientInfo WHERE IsActive = 'Y' AND AD_Client_ID = " + AD_Client_ID;
                    //                    int primaryAccountingSchema = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, null));
                    //                    if (primaryAccountingSchema > 0)
                    //                    {
                    //                        Price = receivedPrice;
                    //                        Qty = receivedQty;
                    //                        AD_Org_ID = AD_Org_ID2;
                    //                        acctSchema = new MAcctSchema(ctx, primaryAccountingSchema, trxName);
                    //                        if (product != null)
                    //                        {
                    //                            #region Costing Level
                    ...
                    //                                        if (pca != null)
                    //                                        {
                    //                                            if (pca.GetFRPT_CostingLevel() == "C" || pca.GetFRPT_CostingLevel() == "B")
                    //                                            {
                    //                                                AD_Org_ID = 0;
                    //                                            }
                    //                                            else if ((string.IsNullOrEmpty(pca.GetFRPT_CostingLevel())) && (acctSchema.GetCostingLevel() == "C" || acctSchema.GetCostingLevel() == "B"))
                    //                                            {
                    //                                                AD_Org_ID = 0;
                    //                                            }
                    //                                        }
                    ...
                    //                        #region check MR matached from PO through Form or not
                    //                        // if yes, then call Match IV
                    //                        if (windowName == "Invoice(Vendor)")
                    //                        {
                    //                            if (product != null)
                    //                            {
                    //                                sql = @"SELECT IsMatchPOForm FROM M_MatchPO WHERE IsActive = 'Y' 
                    //                                     AND M_InoutLine_ID = " + Util.GetValueOfInt(invoiceline.GetM_InOutLine_ID()) +
                    //                                         " AND M_Product_ID = " + product.GetM_Product_ID();
                    //                                isMatchFromForm = Util.GetValueOfString(DB.ExecuteScalar(sql, null, trxName));
                    //                                if (isMatchFromForm == "Y")
                    //                                {
                    //                                    inoutline = new MInOutLine(ctx, invoiceline.GetM_InOutLine_ID(), trxName);
                    //                                    if (inoutline.GetM_InOutLine_ID() > 0)
                    ...
                    //                            sql = "SELECT M_CostDetail_ID FROM M_CostDetail WHERE M_InOutLine_ID = " + inoutline.GetM_InOutLine_ID() +
                    //                                 "  AND c_invoiceline_id IS NULL AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID();
                    //                            costDetailId = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, trxName));
                    //                            cd = new MCostDetail(ctx, costDetailId, trxName);

                    //                            //change 4-5-2016
                    //                            if (cl != "B")
                    //                            {
                    //                                sql = @"SELECT  Amt/Qty as currentCostAmount  FROM m_costelementdetail ced 
                    //                                    where  ced.IsActive = 'Y' AND ced.M_Product_ID = " + product.GetM_Product_ID() + @" 
                    //                                     AND ced.C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID() + @" AND  NVL(ced.M_AttributeSetInstance_ID , 0) = 0" +
                    //                                   " AND ced.M_InOutLine_ID =  " + inoutline.GetM_InOutLine_ID() + @" AND NVL(ced.C_OrderLIne_ID , 0) = 0 AND NVL(ced.C_InvoiceLine_ID , 0) = 0 and 
                    //                                     ced.M_CostElement_ID = (SELECT M_CostElement_ID from M_costElement where costingMethod = 'A' AND IsActive = 'Y' AND AD_Client_ID = " + inoutline.GetAD_Client_ID() + " ) AND ced.AD_Client_ID = " + inoutline.GetAD_Client_ID();
                    ...
                    //                                     ced.M_CostElement_ID = (SELECT M_CostElement_ID from M_costElement where costingMethod = 'A' AND IsActive = 'Y' AND AD_Client_ID = " + inoutline.GetAD_Client_ID() + " ) AND ced.AD_Client_ID = " + inoutline.GetAD_Client_ID();
                    //                            }
                    //                            MRPriceAvPo = Util.GetValueOfDecimal(DB.ExecuteScalar(sql));
                    ...
                    //                                     ced.M_CostElement_ID = (SELECT M_CostElement_ID from M_costElement where costingMethod = 'p' AND IsActive = 'Y' AND AD_Client_ID = " + inoutline.GetAD_Client_ID() + " ) AND ced.AD_Client_ID = " + inoutline.GetAD_Client_ID();
                    //                            }
                    //                            MRPriceLastPO = Util.GetValueOfDecimal(DB.ExecuteScalar(sql));
                    ...
                    //                            if (costDetailId > 0 && cd.GetC_OrderLine_ID() == 0)
                    //                            {
                    //                                // update m_cost Accumulative Amount to be reduced for Av. PO and Last PO
                    //                                sql = "SELECT M_CostElement_ID FROM M_CostElement WHERE  AD_Client_ID=" + AD_Client_ID + " AND IsActive='Y' AND CostElementType='M'  AND CostingMethod = 'A'";
                    //                                int costElementId = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, trxName));
                    //                                // Av. PO
                    //                                if (cl != "B")
                    //                                {
                    //                                    cost = MCost.Get(product, 0, acctSchema, AD_Org_ID, costElementId);
                    ...
                    //                                    cost = MCost.Get(product, M_ASI_ID, acctSchema, AD_Org_ID, costElementId);
                    //                                }
                    //                                //cost.SetCumulatedAmt(Decimal.Subtract(cost.GetCumulatedAmt(), (MRPrice * Qty)));
                    //                                cost.SetCumulatedAmt(Decimal.Subtract(cost.GetCumulatedAmt(), (MRPriceAvPo * Qty)));
                    //                                if (!cost.Save())
                    //                                {
                    //                                    trxName.Rollback();
                    //                                    ValueNamePair pp = VLogger.RetrieveError();
                    //                                    _log.Info("Error occured during saving a record in cost detail -> MatchPO. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                    //                                    return false;
                    //                                }
                    ...
                    //                                    trxName.Rollback();
                    //                                    ValueNamePair pp = VLogger.RetrieveError();
                    //                                    _log.Info("Error occured during saving a record in M-Cost -> MatchPO. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                    //                                    return false;
                    //                                }
                    //                                else
                    //                                {
                    ...
                    //                                // last po
                    //                                sql = "SELECT M_CostElement_ID FROM M_CostElement WHERE  AD_Client_ID=" + AD_Client_ID + " AND IsActive='Y' AND CostElementType='M'  AND CostingMethod = 'p'";
                    //                                costElementId = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, trxName));
                    ...
                    //                                // reduce 
                    //                                //cost.SetCumulatedAmt(Decimal.Subtract(cost.GetCumulatedAmt(), (MRPrice * Qty))); // reduce amount of only MR
                    //                                cost.SetCumulatedAmt(Decimal.Subtract(cost.GetCumulatedAmt(), (MRPriceLastPO * Qty))); // reduce amount of only MR
                    //                                cost.SetCumulatedAmt(Decimal.Add(cost.GetCumulatedAmt(), cd.GetAmt())); // add amount of Match PO
                    //                                cost.SetCurrentCostPrice(Decimal.Round(Price, acctSchema.GetCostingPrecision()));
                    //                                if (!cost.Save())
                    ...
                    //                                     ced.M_CostElement_ID = (SELECT M_CostElement_ID from M_costElement where costingMethod = 'L' AND IsActive = 'Y' AND AD_Client_ID = " + inoutline.GetAD_Client_ID() + " ) AND ced.AD_Client_ID = " + inoutline.GetAD_Client_ID();
                    //                            }
                    //                            MRPriceLifo = Util.GetValueOfDecimal(DB.ExecuteScalar(sql));
                    ...
                    //                                     ced.M_CostElement_ID = (SELECT M_CostElement_ID from M_costElement where costingMethod = 'F' AND IsActive = 'Y' AND AD_Client_ID = " + inoutline.GetAD_Client_ID() + " ) AND ced.AD_Client_ID = " + inoutline.GetAD_Client_ID();
                    //                            }
                    //                            MRPriceFifo = Util.GetValueOfDecimal(DB.ExecuteScalar(sql));
                    ...
                    //                                    trxName.Rollback();
                    //                                    ValueNamePair pp = VLogger.RetrieveError();
                    //                                    _log.Info("Error occured during saving a record in M-Cost -> Match IV. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                    //                                    return false;
                    //                                }
                    ...
                    //                                sql = @"SELECT * FROM T_Temp_CostDetail WHERE  M_InOutLine_ID = " + inoutline.GetM_InOutLine_ID() +
                    //                                            " AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID();
                    //                                DataSet ds2 = DB.ExecuteDataset(sql, null, null);
                    ...
                    //                                            }
                    //                                            //end
                    //                                            price = MCostQueue.CalculateCostQueuePrice(invoice, invoiceline, MRPrice, acctSchema, AD_Client_ID, AD_Org_ID);
                    //                                            sql = "UPDATE m_costqueue SET CurrentCostPrice = " + price + " WHERE M_CostQueue_ID = " + Util.GetValueOfInt(ds2.Tables[0].Rows[b]["M_CostQueue_ID"]);
                    ...
                    //                                else
                    //                                {
                    //                                    sql = @"SELECT il.c_invoiceline_id , il.c_invoice_id ,  ROUND(il.linenetamt/il.qtyinvoiced , 4) AS priceactual , mi.qty ,  SUM(mi.qty) AS matchedqty 
                    //                                          FROM m_inout io INNER JOIN m_inoutline iol ON io.m_inout_id = iol.m_inout_id
                    //                                          INNER JOIN m_matchInv mi ON iol.m_inoutline_id = mi.m_inoutline_id
                    //                                          INNER JOIN c_invoiceline il ON il.c_invoiceline_id = mi.c_invoiceline_id
                    //                                          WHERE mi.isactive      = 'Y' AND io.M_InOut_ID      =" + inoutline.GetM_InOut_ID() +
                    //                                                    " AND iol.m_inoutline_ID = " + inoutline.GetM_InOutLine_ID() +
                    //                                                     " GROUP BY  il.c_invoiceline_id , il.c_invoice_id , ROUND(il.linenetamt/il.qtyinvoiced , 4) , mi.qty ";
                    //                                    ds2 = DB.ExecuteDataset(sql, null, trxName);
                    //                                    if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                    //                                    {
                    //                                        Price = 0;
                    //                                        Qty = 0;
                    ...
                    //                                            if (inv.GetC_Currency_ID() != acctSchema.GetC_Currency_ID())
                    //                                            {
                    //                                                Price += MConversionRate.Convert(ctx, decimal.Multiply(Util.GetValueOfDecimal(ds2.Tables[n].Rows[n]["priceactual"]), Util.GetValueOfDecimal(ds2.Tables[0].Rows[n]["qty"])),
                    //                                                                                 inv.GetC_Currency_ID(), acctSchema.GetC_Currency_ID(),
                    //                                                                                 inv.GetDateAcct(), inv.GetC_ConversionType_ID(), AD_Client_ID, AD_Org_ID);
                    //                                            }
                    //                                            else
                    ...
                    //                                    sql = @"SELECT * FROM T_Temp_CostDetail WHERE  M_InOutLine_ID = " + inoutline.GetM_InOutLine_ID() +
                    //                                              " AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID();
                    //                                    ds2 = DB.ExecuteDataset(sql, null, null);
                    //                                    if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                    //                                    {
                    //                                        for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
                    //                                        {
                    ...
                    //                                            Price = Decimal.Add(Price, Decimal.Multiply(MRPrice, tempQty));
                    //                                            //end
                    //                                            sql = "UPDATE m_costqueue SET CurrentCostPrice = " + Decimal.Round(Price / Qty, acctSchema.GetCostingPrecision()) + " WHERE M_CostQueue_ID = " + Util.GetValueOfInt(ds2.Tables[0].Rows[k]["M_CostQueue_ID"]);
                    //                                            DB.ExecuteQuery(sql, null, trxName);
                    //                                        }
                    //                                    }
                    //                                    ds2.Dispose();
                    ...
                    //                                {
                    //                                    trxName.Rollback();
                    //                                    _log.Info("Error occured during saving a record in M-Cost -> Match IV");
                    //                                    return false;
                    //                                }
                    //                            }
                    //                            else
                    ...
                    //                                if (invoice.GetC_Currency_ID() != acctSchema.GetC_Currency_ID())
                    //                                {
                    //                                    Price = MConversionRate.Convert(ctx, Decimal.Multiply(Qty, Decimal.Divide(invoiceline.GetLineNetAmt() , invoiceline.GetQtyInvoiced())), invoice.GetC_Currency_ID(), acctSchema.GetC_Currency_ID(),
                    //                                                                             invoice.GetDateAcct(), invoice.GetC_ConversionType_ID(), AD_Client_ID, AD_Org_ID);
                    //                                }
                    ...
                    //                                    trxName.Rollback();
                    //                                    ValueNamePair pp = VLogger.RetrieveError();
                    //                                    _log.Info("Error occured during saving a record in cost detail -> Match IV. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                    //                                    return false;
                    //                                }
                    //                                else
                    //                                {
                    ...
                    //                                    {
                    //                                        trxName.Rollback();
                    //                                        _log.Info("Error occured during saving a record in M-Cost -> Product Cost IV");
                    //                                        return false;
                    //                                    }
                    //                                }
                    ...
                    //                                              " AND pp.c_uom_id = " + product.GetC_UOM_ID() + " AND pl.issopricelist = 'N') ORDER BY pl.m_pricelist_id asc";
                    //                                }
                    //                                DataSet dsStdPrice = DB.ExecuteDataset(sql, null, null);
                    //                                if (dsStdPrice != null && dsStdPrice.Tables.Count > 0 && dsStdPrice.Tables[0].Rows.Count > 0)
                    //                                {
                    //                                    price = Util.GetValueOfDecimal(dsStdPrice.Tables[0].Rows[0]["pricestd"]);
                    //                                    C_Currency_ID = Util.GetValueOfInt(dsStdPrice.Tables[0].Rows[0]["c_currency_id"]);
                    ...
                    //                            {
                    //                                cdSourceWarehouse = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                    //                                    inventoryLine, inoutline, movementline, invoiceline, 0, Decimal.Negate(cmPrice), Decimal.Negate(Qty), null, trxName);
                    //                            }
                    //                            else
                    //                            {
                    //                                cdSourceWarehouse = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                    //                                    inventoryLine, inoutline, movementline, invoiceline, 0, Decimal.Negate(Price), Decimal.Negate(Qty), null, trxName);
                    //                            }
                    //                            sql = "SELECT AD_Org_ID FROM M_Warehouse WHERE IsActive = 'Y' AND M_Warehouse_ID = (SELECT M_Warehouse_ID FROM M_Movement WHERE IsActive = 'Y' AND M_Movement_ID = " + movementline.GetM_Movement_ID() + ")";
                    //                            AD_Org_ID = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, trxName));
                    //                            #endregion
                    //                        }
                    ...
                    //                            decimal amt = 0;
                    //                            decimal qntity = 0;
                    //                            sql = "SELECT * FROM C_LandedCostAllocation WHERE  C_InvoiceLine_ID = " + invoiceline.GetC_InvoiceLine_ID();
                    //                            DataSet dsLandedCostAllocation = DB.ExecuteDataset(sql, null, trxName);
                    //                            if (dsLandedCostAllocation != null && dsLandedCostAllocation.Tables.Count > 0 && dsLandedCostAllocation.Tables[0].Rows.Count > 0)
                    //                            {
                    //                                invoice = new MInvoice(ctx, invoiceline.GetC_Invoice_ID(), trxName);
                    //                                for (int lca = 0; lca < dsLandedCostAllocation.Tables[0].Rows.Count; lca++)
                    ...
                    //                                    cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, Util.GetValueOfInt(dsLandedCostAllocation.Tables[0].Rows[lca]["M_Product_ID"]),
                    //                                        Util.GetValueOfInt(dsLandedCostAllocation.Tables[0].Rows[lca]["M_AttributeSetInstance_ID"]), windowName, inventoryLine, inoutline, movementline,
                    //                                     invoiceline, Util.GetValueOfInt(dsLandedCostAllocation.Tables[0].Rows[lca]["M_CostElement_ID"]), amt,
                    //                                     qntity, null, trxName);
                    //                                    if (cd != null)
                    //                                    {
                    //                                        productLca = new MProduct(ctx, Util.GetValueOfInt(dsLandedCostAllocation.Tables[0].Rows[lca]["M_Product_ID"]), trxName);
                    ...
                    //                            {
                    //                                cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                    //                                     inventoryLine, inoutline, movementline, invoiceline, 0, cmPrice, Qty, null, trxName);
                    //                            }
                    //                            else
                    //                            {
                    //                                cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                    //                                         inventoryLine, inoutline, movementline, invoiceline, 0, Price, Qty, null, trxName);
                    //                            }
                    //                        }
                    ...
                    //                                     " ( SELECT MMPolicy FROM M_Product_Category WHERE IsActive = 'Y' AND M_Product_Category_ID = " +
                    //                                     " (SELECT M_Product_Category_ID FROM M_Product WHERE IsActive = 'Y' AND M_Product_ID = " + product.GetM_Product_ID() + " )) AND AD_Client_ID = " + AD_Client_ID;
                    //                            costingElementId = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, null));
                    //                            costElement = new MCostElement(ctx, costingElementId, null);
                    ...
                    //                                {
                    //                                    result = CreateCostQueue(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, inventoryLine.GetAD_Org_ID(), Price / Qty, Qty, windowName,
                    //                                         inventoryLine, inoutline, movementline, invoiceline, cd, trxName);
                    //                                    if (!result)
                    //                                    {
                    //                                        trxName.Rollback();
                    //                                        _log.Info("Error occured during CreateCostQueue.");
                    //                                        return false;
                    //                                    }
                    //                                }
                    //                                else
                    ...
                    //                                {
                    //                                    sql = @"SELECT * FROM T_Temp_CostDetail WHERE C_OrderLine_ID = " + invoiceline.GetC_OrderLine_ID() +
                    //                                         " AND M_InOutLine_ID = " + invoiceline.GetM_InOutLine_ID() + " AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID();
                    //                                    DataSet ds1 = DB.ExecuteDataset(sql, null, trxName);
                    //                                    if (ds1 != null && ds1.Tables.Count > 0 && ds1.Tables[0].Rows.Count > 0)
                    //                                    {
                    //                                        for (int k = 0; k < ds1.Tables[0].Rows.Count; k++)
                    //                                        {
                    ...
                    //                                            // handle partial payment
                    //                                            X_T_Temp_CostDetail tempCostDetail = new X_T_Temp_CostDetail(invoiceline.GetCtx(), Util.GetValueOfInt(ds1.Tables[0].Rows[k]["T_Temp_CostDetail_ID"]), trxName);
                    //                                            price = MCostQueue.CalculateCostQueuePrice(invoice, invoiceline, tempCostDetail.GetAmt(), acctSchema, AD_Client_ID, AD_Org_ID);

                    //                                            //sql = "UPDATE m_costqueue SET CurrentCostPrice = " + cd.GetAmt() / cd.GetQty() + " WHERE M_CostQueue_ID = " + Util.GetValueOfInt(ds1.Tables[0].Rows[k]["M_CostQueue_ID"]);
                    //                                            sql = "UPDATE m_costqueue SET CurrentCostPrice = " + price + " WHERE M_CostQueue_ID = " + Util.GetValueOfInt(ds1.Tables[0].Rows[k]["M_CostQueue_ID"]);
                    //                                            DB.ExecuteQuery(sql, null, trxName);
                    //                                        }
                    //                                    }
                    //                                    ds1.Dispose();
                    ...
                    //                                {
                    //                                    trxName.Rollback();
                    //                                    _log.Info("Error occured during UpdateProductCost.");
                    //                                    return false;
                    //                                }
                    //                            }
                    //                            else if (windowName == "Material Receipt" || windowName == "Customer Return" || windowName == "Shipment" || windowName == "Return To Vendor")
                    ...
                    //                                    if (docType.GetDocBaseType() == "APC")
                    //                                    {
                    //                                        cd.UpdateProductCost("Invoice(APC)", cd, acctSchema, product, M_ASI_ID, invoiceline.GetAD_Org_ID());
                    //                                    }
                    //                                }
                    //                            }
                    //                        }
                    ...
                    //                        {
                    //                            result = cd.CreateCostForCombination(cd, acctSchema, product, M_ASI_ID, 0, windowName);
                    //                        }
                    //                        if (!result)
                    //                        {
                    //                            trxName.Rollback();
                ...
                if (acctSchemaRecord.Length > 0)
                {
                    for (int i = 0; i < acctSchemaRecord.Length; i++)
                    {
                        ...
                        if (windowName == "Match PO")
                        {
                            ...
                            {
                                if (!CreateCostQueueForMatchPO(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, AD_Org_ID,
                                                              Price, inoutline.GetMovementQty(), inoutline, trxName))
                                {
                                    _log.Severe("Error occured during craetion of cost queue -> MatchPO. M_Inoutline_ID is " + inoutline.GetM_InOutLine_ID());
                                    return false;
                                }
        ...
        /// <returns>qty</returns>
        public static decimal CheckQtyAvailablity(Ctx ctx, MAcctSchema acctSchema, int AD_Client_ID, int AD_Org_ID, MProduct product, int M_ASI_ID)
        {
            decimal qty = 0;
            try
            {
                string sql = @"SELECT ROUND(AVG(CST.CURRENTQTY),4 )   FROM M_PRODUCT P   INNER JOIN M_COST CST   ON P.M_PRODUCT_ID=CST.M_PRODUCT_ID
                              ...
                              AND ((   CASE WHEN PC.COSTINGMETHOD IS NOT NULL  AND PC.COSTINGMETHOD   = 'C'  THEN PC.M_costelement_id
                                            WHEN ACC.COSTINGMETHOD IS NOT NULL AND ACC.COSTINGMETHOD   = 'C' THEN ACC.M_costelement_id
                                            WHEN PC.COSTINGMETHOD IS NOT NULL  THEN (SELECT M_CostElement_ID FROM M_costelement 
                                             WHERE COSTINGMETHOD = pc.COSTINGMETHOD AND ad_client_id    = " + AD_Client_ID + @" ) ELSE
                             ...
                             AND ((    CASE WHEN PC.COSTINGLEVEL IS NOT NULL  AND PC.COSTINGLEVEL   IN ('A' , 'O')  THEN " + AD_Org_ID + @"
                                            WHEN ACC.COSTINGLEVEL IS NOT NULL AND ACC.COSTINGLEVEL   IN ('A' , 'O') THEN " + AD_Org_ID + @"
        ...
        public static Decimal GetPrice(MInOutLine inoutline, int CurrencyTo)
        {
            ...
            MInOut inout = new MInOut(inoutline.GetCtx(), inoutline.GetM_InOut_ID(), inoutline.Get_Trx());
        ...
        private static Decimal CalculateCost(Ctx ctx, MAcctSchema acctSchema, MProduct product, int M_ASI_ID, int AD_Client_ID, int AD_Org_ID)
        {
            Decimal price = 0;
            int costElementID = 0;
            string sql = "";
            ...
            try
            {
                ...
                else if (costingLevel == "A") // batch + Org
                {
                    sql = "SELECT CurrentCostPrice FROM M_Cost WHERE IsActive = 'Y' AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID() +
                           " AND M_Product_ID = " + product.GetM_Product_ID() + " AND NVL(M_AttributeSetInstance_ID , 0) = " + M_ASI_ID + " AND  M_CostElement_ID = " + costElementID +
                           " AND AD_Client_ID = " + AD_Client_ID + " AND AD_Org_ID = " + AD_Org_ID;
                }
                else if (costingLevel == "C") // client
                {
                    sql = "SELECT CurrentCostPrice FROM M_Cost WHERE IsActive = 'Y' AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID() +
        ...
        private static bool CreateCostQueueForMatchPO(Ctx ctx, MAcctSchema acctSchema, MProduct product, int M_ASI_ID, int AD_Client_ID,
                                                      int AD_Org_ID, Decimal Price, Decimal Qty, MInOutLine inoutline, Trx trxName, string optionalStrPO = "process")
        {
