...
{
    public class MVABCashJRNL : X_VAB_CashJRNL, DocAction
    {
        #region variables
        /**	Static Logger	*/
        private static VLogger _log = VLogger.GetVLogger(typeof(MVABCashJRNL).FullName);
        /**	Lines					*/
        private MVABCashJRNLLine[] _lines = null;
        /** CashBook				*/
        private MVABCashBook _book = null;
        /**	Process Message 			*/
         ...
         *	@return cash
         */
        public static MVABCashJRNL Get(Ctx ctx, int VAF_Org_ID, DateTime? dateAcct, int VAB_Currency_ID, Trx trxName)
        {
            MVABCashJRNL retValue = null;
            //	Existing Journal
            ...
            try
            {
                ...
                {
                    retValue = new MVABCashJRNL(ctx, dr, trxName);
                }
            }
            catch (Exception e)
            {
            ...
            //	Get CashBook
            MVABCashBook cb = MVABCashBook.Get(ctx, VAF_Org_ID, VAB_Currency_ID);
            if (cb == null)
            {
            ...
            //	Create New Journal
            retValue = new MVABCashJRNL(cb, dateAcct);
         ...
         *	@return cash
         */
        public static MVABCashJRNL Get(Ctx ctx, int VAF_Org_ID, DateTime? dateAcct, int VAB_Currency_ID, int VAPOS_Terminal_ID, Trx trxName)
        {
            MVABCashJRNL retValue = null;
            int cashbook_ID = 0;
            MVABCashBook cb = null;
            ...
            // get cashbook 
            cb = new MVABCashBook(ctx, cashbook_ID, trxName);
         ...
         *	@return cash
         */
        public static MVABCashJRNL Get(Ctx ctx, int VAB_CashBook_ID, DateTime? dateAcct, Trx trxName)
        {
            ...
            //	Get CashBook
            MVABCashBook cb = new MVABCashBook(ctx, VAB_CashBook_ID, trxName);
            if (cb.Get_ID() == 0)
            {
        ...
        /// <returns></returns>
        public static MVABCashJRNL GetCash(Ctx ctx, int VAB_CashBook_ID, DateTime? dateAccount, Trx trxName, int shift, DateTime? ShiftDate)
        {
        ...
        public static MVABCashJRNL Get(Ctx ctx, int VAF_Org_ID, DateTime? dateAcct, int VAB_Currency_ID, int VAB_CashBook_ID, int VAB_DocTypes_ID, Trx trxName)
        {
            MVABCashJRNL retValue = null;
            //	Existing Journal
            String sql = "SELECT * FROM VAB_CashJRNL c "
        ...
        public static MVABCashJRNL Get(Ctx ctx, int VAB_CashBook_ID, DateTime? statementDate, DateTime? dateAcct, Trx trxName)
        {
            MVABCashJRNL retValue = null;
            //	Existing Journal
            String sql = "SELECT * FROM VAB_CashJRNL c "
            ...
            try
            {
                ...
                idr = DB.ExecuteReader(sql, param, trxName);
                if (idr.Read())
                    retValue = new MVABCashJRNL(ctx, idr, trxName);
                idr.Close();
            }
            catch (Exception e)
            {
            ...
            //	Create New Journal
            retValue = new MVABCashJRNL(cb, statementDate);
            retValue.Save(trxName);
            return retValue;
        }
        //End Amit
         ...
         *	@param trxName transaction
         */
        public MVABCashJRNL(Ctx ctx, int VAB_CashJRNL_ID, Trx trxName)
         ...
         *	@param trxName transaction
         */
        public MVABCashJRNL(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
        ...
        *	@param trxName transaction
        */
        public MVABCashJRNL(Ctx ctx, IDataReader dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
         ...
         */
        public MVABCashJRNL(MVABCashBook cb, DateTime? today)
         ...
         *	@return lines
         */
        public MVABCashJRNLLine[] GetLines(bool requery)
        {
            if (_lines != null && !requery)
                return _lines;
            List<MVABCashJRNLLine> list = new List<MVABCashJRNLLine>();
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    list.Add(new MVABCashJRNLLine(GetCtx(), dr, Get_TrxName()));
                }
            }
            catch (Exception e)
            {
            ...
            _lines = new MVABCashJRNLLine[list.Count];
            _lines = list.ToArray();
            return _lines;
        }
        ...
        {
            if (_book == null)
                _book = MVABCashBook.Get(GetCtx(), GetVAB_CashBook_ID());
            return _book;
        }
        ...
        protected override bool BeforeSave(bool newRecord)
        {
            ...
            if (GetVAF_Org_ID() == 0)
            {
                log.SaveError("Error", Msg.ParseTranslation(GetCtx(), "@VAF_Org_ID@"));
                return false;
            }
        ...
        public String PrepareIt()
        {
            ...
            //	Std Period open?
            if (!MPeriod.IsOpen(GetCtx(), GetDateAcct(), MDocBaseType.DOCBASETYPE_CASHJOURNAL, GetVAF_Org_ID()))
            {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    amt = MVABExchangeRate.Convert(GetCtx(), line.GetAmount(),
                    ...
                    if (amt == 0)
                    {
                        ...
                        MVABCurrencyType conv = new MVABCurrencyType(GetCtx(), line.GetVAB_CurrencyType_ID(), Get_TrxName());
                        _processMsg = Msg.GetMsg(GetCtx(), "NoConversion") + MVABCurrency.GetISO_Code(GetCtx(), line.GetVAB_Currency_ID()) + Msg.GetMsg(GetCtx(), "ToCashBookCurr")
                            + MVABCurrency.GetISO_Code(GetCtx(), VAB_Currency_ID) + " - " + Msg.GetMsg(GetCtx(), "ConversionType") + conv.GetName();
                        return DocActionVariables.STATUS_INVALID;
                    }
                    difference = Decimal.Add(difference, amt);
                }
                ...
                {
                    MVABBusinessPartner bp = new MVABBusinessPartner(GetCtx(), line.GetVAB_BusinessPartner_ID(), Get_Trx());
                    string retMsg = "";
        ...
        public String CompleteIt()
        {
            ...
            MVABCashJRNLLine[] Cashlines = GetLines(false);
            int CashTyp_Invc = 0;
            for (int i = 0; i < Cashlines.Length; i++)
            {
                MVABCashJRNLLine Cashline = Cashlines[i];
                // if record is inactive then not to consider 
                if (!Cashline.IsActive())
                    continue;
                if (MVABCashJRNLLine.CASHTYPE_Invoice.Equals(Cashline.GetCashType()))
                {
                    CashTyp_Invc++;
                }
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (MVABCashJRNLLine.CASHTYPE_Invoice.Equals(line.GetCashType()))
                {
                    bool differentCurrency = GetVAB_Currency_ID() != line.GetVAB_Currency_ID();
                    MVABDocAllocation hdr = alloc;
                    if (differentCurrency)
                    {
                        hdr = new MVABDocAllocation(GetCtx(), false,
                    ...
                    //	Allocation Line
                    MVABDocAllocationLine aLine = new MVABDocAllocationLine(hdr, line.GetAmount(),
                ...
                else if (MVABCashJRNLLine.CASHTYPE_BankAccountTransfer.Equals(line.GetCashType()))
                {
                    ...
                    pay.SetVAB_DocTypes_ID(!line.GetVSS_PAYMENTTYPE().Equals(MVABCashJRNLLine.VSS_PAYMENTTYPE_Receipt));
                    ...
                    if (line.GetTransferType() == MVABCashJRNLLine.TRANSFERTYPE_Check)
                    {
                    ...
                    if (Env.IsModuleInstalled("VA009_"))
                    {
                        string qry = "SELECT MAX(VA009_PaymentMethod_ID) FROM VA009_PaymentMethod WHERE IsActive = 'Y' AND VAF_Client_ID = " + GetVAF_Client_ID() + " AND VA009_PaymentBaseType = " +
                            (line.GetTransferType() == MVABCashJRNLLine.TRANSFERTYPE_Check ? "'S'" : "'T'") + " AND VAF_Org_ID IN (0, " + GetVAF_Org_ID() + ")";
                    ...
                    {
                        MVABBankAcct bankAcct = new MVABBankAcct(GetCtx(), line.GetVAB_Bank_Acct_ID(), Get_TrxName());
                        ...
                        {    //Currency conversion in Case of different currencies
                            decimal? amt = Util.GetValueOfDecimal(MVABExchangeRate.Convert(GetCtx(), line.GetAmount(), line.GetVAB_Currency_ID(), bankAcct.GetVAB_Currency_ID(), GetDateAcct(),
                            ...
                            if (amt == null || amt == 0)
                            {
                                ...
                                MVABCurrencyType conv = new MVABCurrencyType(GetCtx(), line.GetVAB_CurrencyType_ID(), Get_TrxName());
                                _processMsg = Msg.GetMsg(GetCtx(), "NoConversion") + MVABCurrency.GetISO_Code(GetCtx(), line.GetVAB_Currency_ID()) + Msg.GetMsg(GetCtx(), "ToBankCurrency")
                                    + MVABCurrency.GetISO_Code(GetCtx(), bankAcct.GetVAB_Currency_ID()) + " - " + Msg.GetMsg(GetCtx(), "ConversionType") + conv.GetName();
                                return DocActionVariables.STATUS_INVALID;
                            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                // Added to Update Open Balance of Business Partner
                else if (MVABCashJRNLLine.CASHTYPE_BusinessPartner.Equals(line.GetCashType()))
                {
                    if (line.GetVAB_BusinessPartner_ID() != 0)
                    {
                        string msg = SetBPOpenbalence(line);
        ...
        {
            MVABDocAllocationLine[] allocLines = GetLinesAllocHeader(HdrID);
            ...
            if (allocLines != null && allocLines.Length > 0)
            {
                ...
                {
                    MVABDocAllocationLine line = allocLines[i];
        ...
        private void UpdateBP(HashSet<int> bps)
        {
            ...
            while (it.MoveNext())
            {
                ...
                MVABBusinessPartner bp = new MVABBusinessPartner(GetCtx(), VAB_BusinessPartner_ID, Get_TrxName());
        ...
        public MVABDocAllocationLine[] GetLinesAllocHeader(int allocID)
        {
            MVABDocAllocationLine[] _Allocline = null;

            String sql = "SELECT * FROM VAB_DocAllocationLine WHERE VAB_DocAllocation_ID=" + allocID;
            List<MVABDocAllocationLine> list = new List<MVABDocAllocationLine>();
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        MVABDocAllocationLine line = new MVABDocAllocationLine(GetCtx(), dr, Get_TrxName());
                        //line.SetParent(this);
                        list.Add(line);
                    }
                }
            ...
            //
            _Allocline = new MVABDocAllocationLine[list.Count];
            _Allocline = list.ToArray();
            return _Allocline;
        }

        //Added By Bharat 07/11/2015 (to Update Open Balance of Business Partner)
        private string SetBPOpenbalence(MVABCashJRNLLine line)
        {
            string errorMsg = "";
            Decimal? UpdatedBal = 0;
            try
            {
                MVABBusinessPartner bp = new MVABBusinessPartner(GetCtx(), line.GetVAB_BusinessPartner_ID(), Get_TrxName());
                ...
                if (bp.GetCreditStatusSettingOn() == "CH")
                {
                    ...
                    Decimal? cashAmt = VAdvantage.Model.MVABExchangeRate.ConvertBase(GetCtx(), Decimal.Add(Decimal.Add(line.GetAmount(), line.GetDiscountAmt()), line.GetWriteOffAmt()),
                    ...
                    if (cashAmt == null || cashAmt == 0)
                    {
                        ...
                        MVABCurrencyType conv = new MVABCurrencyType(GetCtx(), line.GetVAB_CurrencyType_ID(), Get_TrxName());
                        errorMsg = Msg.GetMsg(GetCtx(), "NoConversion") + MVABCurrency.GetISO_Code(GetCtx(), line.GetVAB_Currency_ID()) + Msg.GetMsg(GetCtx(), "ToBaseCurrency")
                ...
                if (bp.GetCreditStatusSettingOn() == "CL")
                {
                    ...
                    if (bp.Save(Get_TrxName()))
                    {
                        ...
                        {
                            loc = new MVABBPartLocation(GetCtx(), line.GetVAB_BPart_Location_ID(), Get_TrxName());
                        }
                        //	Total Balance
        ...
        {
            MVABCashBook cashbook = new MVABCashBook(GetCtx(), GetVAB_CashBook_ID(), Get_TrxName());
