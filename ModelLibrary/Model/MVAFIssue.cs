...
{
    public class MVAFIssue : X_VAF_Issue
    { 
   ...
   public static MVAFIssue Create(LogRecord record)
	{
		...
		if (!DataBase.DB.IsConnected() 
			...
			return null;
		//
		MVAFIssue issue = new MVAFIssue(record);
	...
	public static MVAFIssue Create(Ctx ctx, String hexInput)
	{
		...
		MVAFIssue issue = new MVAFIssue(ctx, (Dictionary<String,String>)hmIn);
		return issue;
	}	//	create
	
	/**	Logger	*/
	private static VLogger _log = VLogger.GetVLogger (typeof(MVAFIssue).FullName);//.class);
	...
	public MVAFIssue (Ctx ctx, int VAF_Issue_ID, Trx trxName):base(ctx, VAF_Issue_ID, trxName)
	{
	...
	/// <param name="trxName">trx</param>
	public MVAFIssue (Ctx ctx,DataRow dr, Trx trxName):base(ctx,dr, trxName)
	{
	...
	public MVAFIssue(LogRecord record):this (Env.GetCtx(), 0, null)
	{
	...
	public MVAFIssue(Ctx ctx,Dictionary<String,String> hmIn):base(ctx, 0, null)
	{
	...
	{
		StringBuilder sb = new StringBuilder ("MVAFIssue[");
		sb.Append (Get_ID())
