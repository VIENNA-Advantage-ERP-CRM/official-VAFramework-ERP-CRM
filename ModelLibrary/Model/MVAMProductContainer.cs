...
{
    public class MVAMProductContainer : X_VAM_ProductContainer
    {
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAMProductContainer).FullName);

        public MVAMProductContainer(Ctx ctx, int VAM_ProductContainer_ID, Trx trxName)
            : base(ctx, VAM_ProductContainer_ID, trxName)
        {
        }
        public MVAMProductContainer(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        ...
        /// <param name="trxName">transaction</param>
        /// <returns>existing - ordered by location priority (desc) and/or guarantee date</returns>
        public static Decimal GetContainerQtyFroMVAMStorage(Ctx Ctx, int VAM_Warehouse_ID, int VAM_Locator_ID, int VAM_ProductContainer_ID,
                                                               int VAM_Product_ID, int VAM_PFeature_SetInstance_ID, int VAM_PFeature_Set_ID,
