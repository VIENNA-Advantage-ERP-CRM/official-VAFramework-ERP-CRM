 ...
 * Class Name     : MVAMProductDownload
 * Purpose        : 
 * Class Used     : 
...
{
    public class MVAMProductDownload : X_VAM_ProductDownload
    {
        /**	Cache						*/
        private static CCache<int, MVAMProductDownload> s_cache
            = new CCache<int, MVAMProductDownload>("VAM_ProductDownload", 20);

        /**	Logger	*/
        private static VLogger _log = VLogger.GetVLogger(typeof(MVAMProductDownload).FullName);
        ...
        public static void MigrateDownloads(Ctx ctx)
        {
            ...
            try
            {
                ...
                while (idr.Read())
                {
                    ...
                    //
                    MVAMProductDownload pdl = new MVAMProductDownload(ctx, 0, null);
         ...
         *	@return MVAMProductDownload
         */
        public static MVAMProductDownload Get(Ctx ctx, int VAM_ProductDownload_ID)
        {
            int key = VAM_ProductDownload_ID;
            MVAMProductDownload retValue = (MVAMProductDownload)s_cache[key];
            if (retValue != null)
                return retValue;
            retValue = new MVAMProductDownload(ctx, VAM_ProductDownload_ID, null);
         ...
         *	@param trxName trx
         */
        public MVAMProductDownload(Ctx ctx, int VAM_ProductDownload_ID,
            Trx trxName)
         ...
         *	@param trxName trx
         */
        public MVAMProductDownload(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        }
        ...
        {
            StringBuilder sb = new StringBuilder("MVAMProductDownload[")
                .Append(Get_ID())
