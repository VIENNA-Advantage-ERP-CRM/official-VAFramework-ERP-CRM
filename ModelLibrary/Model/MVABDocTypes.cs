...
{
    public class MVABDocTypes : X_VAB_DocTypes
    {
        ...
        //	Cache					
        static private CCache<int, MVABDocTypes> s_cache = new CCache<int, MVABDocTypes>("VAB_DocTypes", 20);
        /**	Static Logger	*/
        private static VLogger s_log = VLogger.GetVLogger(typeof(MVABDocTypes).FullName);
        ...
        static public MVABDocTypes[] GetOfDocBaseType(Ctx ctx, string docBaseType)
        {
            List<MVABDocTypes> list = new List<MVABDocTypes>();
            ...
            try
            {
                ...
                for (int i = 0; i < pstmt.Tables[0].Rows.Count; i++)
                {
                    ...
                    list.Add(new MVABDocTypes(ctx, rs, null));
                }
                pstmt = null;
            }
            catch (Exception e)
            ...
            MVABDocTypes[] retValue = new MVABDocTypes[list.Count];
        ...
        /// <param name="ctx">Ctx</param>
        /// <returns>array of doc types</returns>
        public static MVABDocTypes[] GetOfClient(Ctx ctx)
        {
            List<MVABDocTypes> list = new List<MVABDocTypes>();
        ...
        static public MVABDocTypes Get(Ctx ctx, int VAB_DocTypes_ID)
        {
            int key = (int)VAB_DocTypes_ID;
            MVABDocTypes retValue = (MVABDocTypes)s_cache[key];
            if (retValue == null)
            {
                retValue = new MVABDocTypes(ctx, VAB_DocTypes_ID, null);
        ...
        public MVABDocTypes(Ctx ctx, int VAB_DocTypes_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVABDocTypes(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        }
        ...
        /// <param name="trxName">transaction</param>
        public MVABDocTypes(Ctx ctx, string docBaseType, string name, Trx trxName)
            : this(ctx, 0, trxName)
        {
            SetVAF_Org_ID(0);
        ...
        {
            bool isSOTrx = MVABMasterDocType.DOCBASETYPE_SALESORDER.Equals(GetDocBaseType())
                || MVABMasterDocType.DOCBASETYPE_MATERIALDELIVERY.Equals(GetDocBaseType())
                || (MVABMasterDocType.DOCBASETYPE_BLANKETSALESORDER.Equals(GetDocBaseType()) && GetValue()=="BSO")
        ...
        {
            StringBuilder sb = new StringBuilder("MVABDocTypes[");
        ...
        public bool IsQuotation()
        {
            return DOCSUBTYPESO_Quotation.Equals(GetDocSubTypeSO())
                && MVABMasterDocType.DOCBASETYPE_SALESORDER.Equals(GetDocBaseType());
        }
