...
{
    public class MVAMCostForeignCurrency : X_VAM_Cost_ForeignCurrency
    {

        private static VLogger _log = VLogger.GetVLogger(typeof(MVAMCostForeignCurrency).FullName);


        public MVAMCostForeignCurrency(Ctx ctx, int VAM_ProductCost_ForeignCurrency_ID, Trx trxName)
            : base(ctx, VAM_ProductCost_ForeignCurrency_ID, trxName)
        {

        }

        public MVAMCostForeignCurrency(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
        ...
        public MVAMCostForeignCurrency(int VAF_Org_ID, MVAMProduct product, int VAM_PFeature_SetInstance_ID,
        ...
        public static MVAMCostForeignCurrency Get(MVAMProduct product, int VAM_PFeature_SetInstance_ID, int VAF_Org_ID, int VAM_ProductCostElement_ID,
            int VAB_BusinessPartner_ID, int VAB_Currency_ID)
        {
            MVAMCostForeignCurrency foreignCurrency = null;
            String sql = "SELECT * "
            ...
            try
            {
                ...
                {
                    foreignCurrency = new MVAMCostForeignCurrency(product.GetCtx(), dr, product.Get_TrxName());
                }
            }
            catch (Exception e)
            {
            ...
            if (foreignCurrency == null)
                foreignCurrency = new MVAMCostForeignCurrency(VAF_Org_ID, product, VAM_PFeature_SetInstance_ID,
        ...
        public static bool InsertForeignCostAverageInvoice(Ctx ctx, MVABInvoice invoice, MVABInvoiceLine invoiceLine, Trx trx)
        {
            ...
            MVAMCostForeignCurrency foreignCost = null;
            dynamic pc = null;
            String cl = null;
            try
            {
                ...
                if (acctSchema.GetVAB_Currency_ID() != invoice.GetVAB_Currency_ID())
                {
                    ...
                    if (product != null && product.GetProductType() == "I" && product.GetVAM_Product_ID() > 0) // for Item Type product
                    {
                        ...
                        foreignCost = MVAMCostForeignCurrency.Get(product, M_ASI_ID, VAF_Org_ID, VAM_ProductCostElement_ID, invoice.GetVAB_BusinessPartner_ID(), invoice.GetVAB_Currency_ID());
            ...
            try
            {
                ...
                if (acctSchema.GetVAB_Currency_ID() != order.GetVAB_Currency_ID())
                {
                    ...
                    if (product != null && product.GetProductType() == "I" && product.GetVAM_Product_ID() > 0) // for Item Type product
                    {
                        ...
                        foreignCost = MVAMCostForeignCurrency.Get(product, M_ASI_ID, VAF_Org_ID, VAM_ProductCostElement_ID, order.GetVAB_BusinessPartner_ID(), order.GetVAB_Currency_ID());
