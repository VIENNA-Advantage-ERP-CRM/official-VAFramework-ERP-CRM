...
namespace VAdvantage.Model
{
    ...
    /// Commission Run
    /// </summary>
    public class MVABWorkCommissionCalc : X_VAB_WorkCommission_Calc
    {
        /// <summary>
        ...
        public MVABWorkCommissionCalc(Ctx ctx, int VAB_WorkCommission_Calc_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVABWorkCommissionCalc(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
        }
        ...
        public MVABWorkCommissionCalc(MVABWorkCommission commission)
        ...
        public MVABWorkCommissionAmt[] GetAmts()
        {
            String sql = "SELECT * FROM VAB_WorkCommission_Amt WHERE VAB_WorkCommission_Calc_ID=@crunid";
            List<MVABWorkCommissionAmt> list = new List<MVABWorkCommissionAmt>();
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        list.Add(new MVABWorkCommissionAmt(GetCtx(), dr, Get_TrxName()));
                    }
                }
            }
            catch (Exception e)
            ...
            //	Convert
            MVABWorkCommissionAmt[] retValue = new MVABWorkCommissionAmt[list.Count];
        ...
        {
            MVABWorkCommissionAmt[] amts = GetAmts();
            Decimal grandTotal = Env.ZERO;
            for (int i = 0; i < amts.Length; i++)
            {
                MVABWorkCommissionAmt amt = amts[i];
