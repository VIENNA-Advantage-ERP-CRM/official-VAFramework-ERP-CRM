...
using System;using System.Text;using VAdvantage.DataBase;using VAdvantage.Common;using VAdvantage.Classes;using VAdvantage.Process;using VAdvantage.Model;using VAdvantage.Utility;using System.Data;/** Generated Model for C_RfQ
 *  @author Raghu (Updated) 
 *  @version Vienna Framework 1.1.1 - $Id$ */
public class X_C_RfQ : PO{public X_C_RfQ (Context ctx, int C_RfQ_ID, Trx trxName) : base (ctx, C_RfQ_ID, trxName){/** if (C_RfQ_ID == 0){SetC_Currency_ID (0);// @$C_Currency_ID @
...
SetQuoteType (null);// S
SetSalesRep_ID (0);} */
}public X_C_RfQ (Ctx ctx, int C_RfQ_ID, Trx trxName) : base (ctx, C_RfQ_ID, trxName){/** if (C_RfQ_ID == 0){SetC_Currency_ID (0);// @$C_Currency_ID @
SetC_RfQ_ID (0);SetC_RfQ_Topic_ID (0);SetDateResponse (DateTime.Now);SetDocumentNo (null);SetIsInvitedVendorsOnly (false);SetIsQuoteAllQty (false);SetIsQuoteTotalAmt (false);SetIsRfQResponseAccepted (true);// Y
SetIsSelfService (true);// Y
SetName (null);SetProcessed (false);// N
SetQuoteType (null);// S
...
static long serialVersionUID = 27829052846336L;/** Last Updated Timestamp 1/8/2019 11:35:29 AM */
public static long updatedMS = 1546927529547L;/** AD_Table_ID=677 */
...
public String GetHelp() {return (String)Get_Value("Help");}/** Set Invited Vendors Only.
...
public void SetM_Requisition_ID (int M_Requisition_ID){if (M_Requisition_ID <= 0) Set_Value ("M_Requisition_ID", null);else
Set_Value ("M_Requisition_ID", M_Requisition_ID);}/** Get Requisition.
...
public void SetQuoteType (String QuoteType){if (QuoteType == null) throw new ArgumentException ("QuoteType is mandatory");if (!IsQuoteTypeValid(QuoteType))
throw new ArgumentException ("QuoteType Invalid value - " + QuoteType + " - Reference_ID=314 - A - S - T");if (QuoteType.Length > 1){log.Warning("Length > 1 - truncated");QuoteType = QuoteType.Substring(0,1);}Set_Value ("QuoteType", QuoteType);}/** Get RfQ Type.
@return Request for Quotation Type */
public String GetQuoteType() {return (String)Get_Value("QuoteType");}/** Set Rank RfQ.
