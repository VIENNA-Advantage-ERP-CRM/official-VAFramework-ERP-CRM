...
{
    [Serializable]
    public class MVABCountry : X_VAB_Country, IComparer<PO>
    {
        #region Variables
        //	Translated Name			
        ...
        private static CCache<String, MVABCountry> s_countries = null;
        //	Default Country 		
        private static MVABCountry _default = null;
        //	Static Logger			
        private static VLogger _log = VLogger.GetVLogger(typeof(MVABCountry).FullName);
        ...
        public static MVABCountry Get(Ctx ctx, int VAB_Country_ID)
        {
            ...
            MVABCountry c = (MVABCountry)s_countries[key];
            if (c != null)
                return c;
            c = new MVABCountry(ctx, VAB_Country_ID, null);
        ...
        /// <returns>Country</returns>
        public static MVABCountry GetDefault(Ctx ctx)
        {
            if (s_countries == null || s_countries.Count == 0)
            {
                LoadAllCountries(ctx);
        ...
        public static MVABCountry[] GetCountries(Ctx ctx)
        {
            if (s_countries == null || s_countries.Count == 0)
            {
                LoadAllCountries(ctx);
            }
            MVABCountry[] retValue = new MVABCountry[s_countries.Count];
            retValue = s_countries.Values.ToArray();
            Array.Sort(retValue, new MVABCountry(ctx, 0, null));
            return retValue;
        }
        ...
        private static void LoadAllCountries(Ctx ctx)
        {
            ...
            MVABCountry usa = null;
            //

            int countryID = Util.GetValueOfInt(ctx.Get("P|VAB_Country_ID"));

            s_countries = new CCache<String, MVABCountry>("VAB_Country", 250);
            ...
            try
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    MVABCountry c = new MVABCountry(ctx, dr, null);
        ...
        public MVABCountry(Ctx ctx, int VAB_Country_ID, Trx trxName)
        ...
        /// <param name="dr"></param>
        /// <param name="trxName"></param>
        public MVABCountry(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
