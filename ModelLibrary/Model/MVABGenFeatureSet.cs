...
{
    public class MVABGenFeatureSet: X_VAB_GenFeatureSet
    {

        //	Instance Attributes					
        private MVABGenFeature[] _instanceAttributes = null;
        //	Instance Attributes					
        private MVABGenFeature[] _productAttributes = null;

        public MVABGenFeatureSet(Ctx ctx, int VAB_GenFeatureSet_ID, Trx trxName)
        ...
        public MVABGenFeatureSet(Ctx ctx, IDataReader idr, Trx trxName)
     ...
     */
        public MVABGenFeature[] GetCGenAttributes(bool instanceAttributes)
        {
                ...
                || _productAttributes == null && !instanceAttributes)
            {
                ...
                List<MVABGenFeature> list = new List<MVABGenFeature>();
                DataTable dt = null;
                ...
                try
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        MVABGenFeature ma = new MVABGenFeature(GetCtx(), Convert.ToInt32(dr[0]), Get_TrxName());
                        list.Add(ma);
                    }
                }
                catch (Exception ex)
                ...
                {
                    _instanceAttributes = new MVABGenFeature[list.Count];
                    _instanceAttributes = list.ToArray();
                }
                else
                {
                    _productAttributes = new MVABGenFeature[list.Count];
