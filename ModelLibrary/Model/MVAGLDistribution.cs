...
{
    public class MVAGLDistribution : X_VAGL_Distribution
    {

        /**	Static Logger	*/
	private static VLogger	_log	= VLogger.GetVLogger (typeof(MVAGLDistribution).FullName);
  /// <summary>
  ...
  /// <param name="VAB_DocTypes_ID">only document type</param>
	...
	public static MVAGLDistribution[] Get (MVABAccount acct,  
	...
	public static MVAGLDistribution[] Get (Ctx ctx, int VAB_AccountBook_ID, 
		String PostingType, int VAB_DocTypes_ID,
		...
		int User1_ID, int User2_ID)
	{
		...
		//
		//ArrayList<MVAGLDistribution> list = new ArrayList<MVAGLDistribution>();
        List<MVAGLDistribution> list=new List<MVAGLDistribution>();
		for (int i = 0; i < acctList.Length; i++)
		{
			MVAGLDistribution distribution = acctList[i];
		...
		//
		MVAGLDistribution[] retValue = new MVAGLDistribution[list.Count];
    ...
    /// <returns>array of distributions</returns>
	public static MVAGLDistribution[] Get (Ctx ctx, int Account_ID)
	{
		//Integer key = new Integer (Account_ID);
        int key=Account_ID;
		//MVAGLDistribution[] retValue = (MVAGLDistribution[])s_accounts.get(key);
        MVAGLDistribution[] retValue = (MVAGLDistribution[])s_accounts[key];
		if (retValue != null) 
        {  
			return retValue;
		}
		...
		try
		{
            ...
            {
				list.Add (new MVAGLDistribution (ctx, dr, null));
            }
		}
		catch (Exception e)
		{
		...
		//
        retValue = new MVAGLDistribution[list.Count];
		//list.toArray (retValue);
        retValue = list.ToArray();
	...
	private static CCache<int,MVAGLDistribution[]> s_accounts 
		= new CCache<int,MVAGLDistribution[]>("VAGL_Distribution", 100);
	...
	public MVAGLDistribution (Ctx ctx, int VAGL_Distribution_ID, Trx trxName):base(ctx, VAGL_Distribution_ID, trxName)
	{
	...
	/// <param name="trxName">transaction</param>
	public MVAGLDistribution (Ctx ctx, DataRow dr, Trx trxName):base(ctx, dr, trxName)
	{
		//super(ctx, rs, trxName);
	}	//	MVAGLDistribution

	/**	The Lines						*/
	private MVAGLDistributionLine[]		_lines = null;
	...
	public MVAGLDistributionLine[] GetLines (Boolean reload)
	{
		...
		//ArrayList<MVAGLDistributionLine> list = new ArrayList<MVAGLDistributionLine>();
        List<MVAGLDistributionLine> list=new List<MVAGLDistributionLine>();
		...
		try
		{
            ...
            foreach(DataRow dr in dt.Rows)
			{
				MVAGLDistributionLine dl = new MVAGLDistributionLine (GetCtx(),dr, Get_TrxName());
				if (dl.IsActive())
				{
		...
		//	return
		_lines = new MVAGLDistributionLine[list.Count];
		//list.toArray (_lines);
        _lines=list.ToArray();
		return _lines;
	}	//	getLines
	...
	public void Distribute (MVABAccount acct, Decimal Amt, int VAB_Currency_ID)
	{
		...
		for (int i = 0; i < _lines.Length; i++)
		{
			MVAGLDistributionLine dl = _lines[i];
			if (!dl.IsActive())
            {
				continue;
            }
