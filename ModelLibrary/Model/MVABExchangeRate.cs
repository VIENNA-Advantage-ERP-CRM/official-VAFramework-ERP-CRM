...
{
    public class MVABExchangeRate : X_VAB_ExchangeRate
    {
        TimeSpan ts = (DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc));
        //Logger					
        private static VLogger _log = VLogger.GetVLogger(typeof(MVABExchangeRate).FullName);
        ...
        public MVABExchangeRate(Ctx ctx, int VAB_ExchangeRate_ID, Trx trxName)
        ...
        /// <param name="trxName">transaction</param>
        public MVABExchangeRate(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
            UpdateFromServer = true;
        }
        ...
        public MVABExchangeRate(PO po, int VAB_CurrencyType_ID, int VAB_Currency_ID, int VAB_Currency_To_ID,
            ...
            int VAF_Client_ID, int VAF_Org_ID)
        {
            ...
            int stdPrecision = MVABCurrency.GetStdPrecision(ctx, CurTo_ID);
            ...
            DateTime? convDate, int ConversionType_ID, int VAF_Client_ID, int VAF_Org_ID)
        {
            ...
            if (VAB_CurrencyType_ID == 0)
                VAB_CurrencyType_ID = MVABCurrencyType.GetDefault(VAF_Client_ID);
            //	Conversion Date
            if (convDate == null)
            {
        ...
        protected override bool AfterSave(bool newRecord, bool success)
        {
            ...
            if (newRecord)
            {
                ...
                if (Util.GetValueOfInt(DB.ExecuteScalar(sql, null, Get_Trx())) <= 0)
                {
                    MVABExchangeRate conRate = new MVABExchangeRate(GetCtx(), 0, null);
