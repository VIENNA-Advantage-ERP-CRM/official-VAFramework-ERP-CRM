 ...
 * Module Name    : 
 * Purpose        : 
 * Class Used     : X_VAB_DocAllocation
...
{
    public class MVABDocAllocation : X_VAB_DocAllocation, DocAction
    {
        /**	Logger						*/
        private static VLogger _log = VLogger.GetVLogger(typeof(MVABDocAllocation).FullName);
        /**	Process Message 			*/
        private String _processMsg = null;
        /**	Just Prepared Flag			*/
        private bool _justPrepared = false;
        /**	Lines						*/
        private MVABDocAllocationLine[] _lines = null;
        /** PaidAmt                     */
        decimal PaidAmt = 0;
        /** variance                    */
        decimal Variance = 0;
        ...
        /// <param name="VAB_DocAllocation_ID">id</param>
        /// <param name="trxName">transaction</param>
        public MVABDocAllocation(Ctx ctx, int VAB_DocAllocation_ID, Trx trxName)
            : base(ctx, VAB_DocAllocation_ID, trxName)
        {
            if (VAB_DocAllocation_ID == 0)
            {
                //	setDocumentNo (null);
        ...
        /// <param name="trxName">transaction</param>
        public MVABDocAllocation(Ctx ctx, DataRow dr, Trx trxName)
            : base(ctx, dr, trxName)
        {
        }
        ...
        /// <param name="trxName">transaction</param>
        public MVABDocAllocation(Ctx ctx, bool IsManual, DateTime? dateTrx, int VAB_Currency_ID,
        ...
        public static MVABDocAllocation[] GetOfPayment(Ctx ctx, int VAB_Payment_ID, Trx trxName)
        {
            ...
            List<MVABDocAllocation> list = new List<MVABDocAllocation>();
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        list.Add(new MVABDocAllocation(ctx, dr, trxName));
                    }
                }
            }
            catch (Exception e)
            ...
            MVABDocAllocation[] retValue = new MVABDocAllocation[list.Count];
        ...
        public static MVABDocAllocation[] GetOfInvoice(Ctx ctx, int VAB_Invoice_ID, Trx trxName)
        {
            String sql = "SELECT * FROM VAB_DocAllocation h "
                + "WHERE IsActive='Y'"
                + " AND EXISTS (SELECT * FROM VAB_DocAllocationLine l "
        ...
        public MVABDocAllocationLine[] GetLines(bool requery)
        {
            if (_lines != null && _lines.Length != 0 && !requery)
                return _lines;
            //
            String sql = "SELECT * FROM VAB_DocAllocationLine WHERE VAB_DocAllocation_ID=" + GetVAB_DocAllocation_ID();
            ...
            //
            _lines = new MVABDocAllocationLine[list.Count];
            _lines = list.ToArray();
            return _lines;
        }
        ...
        public new void SetProcessed(bool processed)
        {
            ...
            if (Get_ID() == 0)
                return;
            String sql = "UPDATE VAB_DocAllocation SET Processed='"
                + (processed ? "Y" : "N")
                + "' WHERE VAB_DocAllocation_ID=" + GetVAB_DocAllocation_ID();
        ...
        protected override bool BeforeDelete()
        {
            ...
            String sql = "UPDATE VAB_DocAllocation SET IsActive='N' WHERE VAB_DocAllocation_ID=" + GetVAB_DocAllocation_ID();
            ...
            {
                MVABDocAllocationLine line = _lines[i];
                bps.Add(line.GetVAB_BusinessPartner_ID());
            ...
            for (int i = 0; i < _lines.Length; i++)
            {
                ...
                if (Env.IsModuleInstalled("VA009_"))
                {
                    if (line.GetVAB_Invoice_ID() > 0 && line.GetVAB_sched_InvoicePayment_ID() > 0)
                    {
                        ...
                        else if (cashline != null && cashline.GetVAB_CashJRNLLine_ID() != 0)
                        {
                            // to get Multiply Rate 
                            currencymultiplyRate = GetCurrencyMultiplyRate(invoice, payment, cashline, journalline);
                        }
                        ...
                        if (BaseCurrency != invoice.GetVAB_Currency_ID())
                        {
                            ...
                            multiplyRate = MVABExchangeRate.GetRate(invoice.GetVAB_Currency_ID(), BaseCurrency, invoice.GetDateAcct(), invoice.GetVAB_CurrencyType_ID(), invoice.GetVAF_Client_ID(), invoice.GetVAF_Org_ID());
                        ...
                        paySch.SetVA009_OpenAmnt(BaseCurrency != invoice.GetVAB_Currency_ID() ? (paySch.GetDueAmt() * multiplyRate) : paySch.GetDueAmt());
                        ...
                        if (line.GetOverUnderAmt() != 0 && !_isDuplicateLine)
                        {
                            ...
                            newPaySch.SetVA009_OpenAmnt(BaseCurrency != invoice.GetVAB_Currency_ID() ? Decimal.Multiply(newPaySch.GetDueAmt(), multiplyRate) : newPaySch.GetDueAmt());
        ...
        {
            return Msg.GetElement(GetCtx(), "VAB_DocAllocation_ID") + " " + GetDocumentNo();
        }
        ...
        private bool ReverseIt()
        {
            ...
            // is Non Business Day?
            // JID_1205: At the trx, need to check any non business day in that org. if not fund then check * org.
            if (MVABNonBusinessDay.IsNonBusinessDay(GetCtx(), GetDateTrx(), GetVAF_Org_ID()))
            {
            ...
            //	Delete Posting
            String sql = "DELETE FROM Actual_Acct_Detail WHERE VAF_TableView_ID=" + MVABDocAllocation.Table_ID
            ...
            if (Env.IsModuleInstalled("VA009_"))
            {
                ...
                sql = "UPDATE va009_batchlinedetails SET  VAB_DocAllocation_ID = null WHERE VAB_DocAllocation_ID = " + GetVAB_DocAllocation_ID();
        ...
        private decimal GetCurrencyMultiplyRate(MInvoice invoice, MPayment payment, MVABCashJRNLLine cashline, MJournalLine journalline)
        {
            ...
            if (GetVAB_Currency_ID() != invoice.GetVAB_Currency_ID())
            {
                ...
                {
                    currencymultiplyRate = MVABExchangeRate.GetRate(GetVAB_Currency_ID(), invoice.GetVAB_Currency_ID(), conversionDate, GetVAB_CurrencyType_ID(), invoice.GetVAF_Client_ID(), invoice.GetVAF_Org_ID());
                }
                else
                {
                    //In case of Journal
                    if (journalline != null)
                    {
                        ...
                        if (journalline != null && invoice != null) // journal to invoice
                        {
                            ...
                            VAB_CurrencyType_ID = GetVAB_CurrencyType_ID();
                            currencyTo_ID = invoice.GetVAB_Currency_ID();
                        ...
                        else if (journalline != null && payment != null) //Journal to payment
                        {
                            DateAcct = conversionDate;
                            VAB_CurrencyType_ID = GetVAB_CurrencyType_ID();
                            currencyTo_ID = payment.GetVAB_Currency_ID();
                        ...
                        {
                            //cash = new MCash(cashline.GetCtx(), cashline.GetVAB_CashJRNL_ID(), cashline.Get_Trx());
                            DateAcct = conversionDate;
                            VAB_CurrencyType_ID = GetVAB_CurrencyType_ID();
                            currencyTo_ID = cash.GetVAB_Currency_ID();
                            VAF_Client_ID = cash.GetVAF_Client_ID();
                            VAF_Org_ID = cash.GetVAF_Org_ID();
                        }
                        return currencymultiplyRate = MVABExchangeRate.GetRate(GetVAB_Currency_ID(), currencyTo_ID, DateAcct, VAB_CurrencyType_ID, VAF_Client_ID, VAF_Org_ID);
                    }
