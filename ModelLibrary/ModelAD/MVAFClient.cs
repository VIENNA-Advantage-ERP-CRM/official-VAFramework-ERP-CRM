...
{
    public class MVAFClient : VAModelAD.Model.MVAFClient
    {
        //Client Info					
        private MVAFClientDetail _info = null;
        /**	Cache						*/
        private static CCache<int, MVAFClient> s_cache = new CCache<int, MVAFClient>("VAF_Client", 3);

        private static VLogger s_log = VLogger.GetVLogger(typeof(MVAFClient).FullName);

        public MVAFClient(Ctx ctx, int VAF_Client_ID, Trx trxName)
        ...
        public static MVAFClient Get(Ctx ctx)
        {
            return Get(ctx, ctx.GetVAF_Client_ID());
        }

        public static MVAFClient Get(Ctx ctx, int VAF_Client_ID)
        {
            int key = VAF_Client_ID;
            MVAFClient client = (MVAFClient)s_cache[key];
            if (client != null)
                return client;
            client = new MVAFClient(ctx, VAF_Client_ID, null);
            if (VAF_Client_ID == 0)
        ...
        public static MVAFClient[] GetAll(Ctx ctx)
        {
            List<MVAFClient> list = new List<MVAFClient>();
            ...
            try
            {
                ...
                {
                    MVAFClient client = new MVAFClient(ctx, dr, null);
                    list.Add(client);
                }
            }
            catch (Exception e)
            ...
            MVAFClient[] RetValue = new MVAFClient[list.Count()];
            RetValue = list.ToArray();
            return RetValue;
        }   //	getAll

        public MVAFClient(Ctx ctx, DataRow rs, Trx trxName)
            : base(ctx, rs, trxName)
        {
        }
        ...
        public MVAFClient(Ctx ctx, int VAF_Client_ID, bool createNew, Trx trxName)
  ...
  //      }	//	
	 ...
	 */
        ...
        public MVAFClientDetail GetInfo()
        {
            if (_info == null)
                _info = MVAFClientDetail.Get(GetCtx(), GetVAF_Client_ID(), Get_Trx());
            return _info;
        }
        ...
        public bool SetupClientInfo(String language)
        {
            ...
            //  Tree IDs
            int VAF_Tree_Org_ID = 0, VAF_Tree_BPartner_ID = 0, VAF_Tree_Project_ID = 0,
                VAF_Tree_SalesRegion_ID = 0, VAF_Tree_Product_ID = 0,
                VAF_Tree_Campaign_ID = 0, VAF_Tree_Activity_ID = 0;
            ...
            try
            {
                ...
                for (int i = 0; i <= dr.Tables[0].Rows.Count - 1; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (treeType == "CC" || treeType == "CM" || treeType == "CS")
                            continue;
                        ...
                        else if (treeType.Equals(X_VAF_TreeInfo.TREETYPE_ElementValue))
                        {
                            ...
                            //success = tree.Save();
                            //m_VAF_Tree_Account_ID = tree.GetVAF_TreeInfo_ID();
                        }
                        else if (treeType.Equals(X_VAF_TreeInfo.TREETYPE_Campaign))
                        {
                            tree = new MVAFTreeInfo(this, name, treeType);
                            success = tree.Save();
                            VAF_Tree_Campaign_ID = tree.GetVAF_TreeInfo_ID();
                        }
                        else if (treeType.Equals(X_VAF_TreeInfo.TREETYPE_Activity))
                        {
                            tree = new MVAFTreeInfo(this, name, treeType);
                            success = tree.Save();
                            VAF_Tree_Activity_ID = tree.GetVAF_TreeInfo_ID();
                        }
            ...
            //	Create ClientInfo
            MVAFClientDetail clientInfo = new MVAFClientDetail(this,
                VAF_Tree_Org_ID, VAF_Tree_BPartner_ID, VAF_Tree_Project_ID,
                VAF_Tree_SalesRegion_ID, VAF_Tree_Product_ID,
                VAF_Tree_Campaign_ID, VAF_Tree_Activity_ID, Get_TrxName());
        ...
        public int GetSetup_VAF_Tree_Account_ID()
        {
            return m_VAF_Tree_Account_ID;
        }	//	getSetup_VAF_Tree_Account_ID
