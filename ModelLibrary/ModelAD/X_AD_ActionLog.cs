...
using System;using System.Text;using VAdvantage.DataBase;using VAdvantage.Common;using VAdvantage.Classes;using VAdvantage.Process;using VAdvantage.Model;using VAdvantage.Utility;using System.Data;/** Generated Model for AD_ActionLog
 *  @author Raghu (Updated) 
 *  @version Vienna Framework 1.1.1 - $Id$ */
public class X_AD_ActionLog : PO{public X_AD_ActionLog (Context ctx, int AD_ActionLog_ID, Trx trxName) : base (ctx, AD_ActionLog_ID, trxName){/** if (AD_ActionLog_ID == 0){SetAD_ActionLog_ID (0);} */
}public X_AD_ActionLog (Ctx ctx, int AD_ActionLog_ID, Trx trxName) : base (ctx, AD_ActionLog_ID, trxName){/** if (AD_ActionLog_ID == 0){SetAD_ActionLog_ID (0);} */
}/** Load Constructor 
@param ctx context
@param rs result set 
...
static long serialVersionUID = 27882884308023L;/** Last Updated Timestamp 9/22/2020 7:16:31 AM */
public static long updatedMS = 1600758991234L;/** AD_Table_ID=1001065 */
...
public void SetAction (String Action){if (!IsActionValid(Action))
throw new ArgumentException ("Action Invalid value - " + Action + " - Reference_ID=104 - B - F - P - R - T - W - X");if (Action != null && Action.Length > 1){log.Warning("Length > 1 - truncated");Action = Action.Substring(0,1);}Set_Value ("Action", Action);}/** Get Action.
...
public void SetActionType (String ActionType){if (!IsActionTypeValid(ActionType))
throw new ArgumentException ("ActionType Invalid value - " + ActionType + " - Reference_ID=1000491 - D - V");if (ActionType != null && ActionType.Length > 1){log.Warning("Length > 1 - truncated");ActionType = ActionType.Substring(0,1);}Set_Value ("ActionType", ActionType);}/** Get ActionType.
@return ActionType */
