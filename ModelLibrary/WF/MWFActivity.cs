            ...
            else if (MWFNode.ACTION_UserChoice.Equals(action))
            {
                ...
                if (_node.IsUserApproval() && (GetPO().GetType() == typeof(DocAction) || GetPO().GetType().GetInterface("DocAction") == typeof(DocAction)))
                {
                    ...
                    else
                    {
                        ...
                        PushNotification.SendNotificationToUser(GetAD_User_ID(), GetAD_Window_ID(), GetRecord_ID(), Msg.GetMsg(GetCtx(), "VA074_WorkFlow"), GetNodeName() + " : " + Msg.GetMsg(GetCtx(), "ReceivedFor") + " " + GetWindowName() + GetDocumentNo(), "W");
                    }
                }	//	approval
                ...
                else if (_node.IsMultiApproval() && _node.GetApprovalLeval() > -1) //For Leave Request Approval
                {
                    ...
                    else if (_node.GetAD_WF_Responsible_ID() > 0 || _node.GetWorkflow().GetAD_WF_Responsible_ID() > 0)
                    {
                        ...
                        if (!resp.IsInvoker() || resp.IsOrganization() || resp.IsSQL())
                        {
                            ...
                            // VIS264 - Send push notification                            
                            PushNotification.SendNotificationToUser(nextAD_User_ID, GetAD_Window_ID(), GetRecord_ID(), Msg.GetMsg(GetCtx(), "VA074_WorkFlow"), _node.GetName() + " : " + Msg.GetMsg(GetCtx(), "ReceivedFor") + " " + GetWindowName() + GetDocumentNo(), "W");
                    ...
                    // VIS264 - Send push notification
                    PushNotification.SendNotificationToUser(nextAD_User_ID, GetAD_Window_ID(), GetRecord_ID(), Msg.GetMsg(GetCtx(), "VA074_WorkFlow"), GetNodeName() + " : " + Msg.GetMsg(GetCtx(), "ReceivedFor") + " " + GetWindowName() + GetDocumentNo(), "W");
...
";
        ...
        public bool ForwardTo(int AD_User_ID, String textMsg, bool isFromActivityForm = false, bool isForwarded = false)
        {
            ...
            // VIS264 - Send push notification
            PushNotification.SendNotificationToUser(GetAD_User_ID(), GetAD_Window_ID(), GetRecord_ID(), Msg.GetMsg(GetCtx(), "VA074_WorkFlow") + " (" + Msg.GetMsg(GetCtx(), "Forwarded") + ")", GetNodeName() + " : " + Msg.GetMsg(GetCtx(), "ReceivedFor") + " " + GetWindowName() + GetDocumentNo(), "W");
